/**
 * HTML QA report generator (CommonJS) — writes to reports/
 */
const fs = require('fs');
const path = require('path');

function buildReport(suites, meta = {}) {
...
function writeHtmlReport(report, outDir = path.join(process.cwd(), 'reports')) {
...
<html lang="en"><head><meta charset="utf-8"/><title>VSP QA Report</title>
<style>
body{font-family:system-ui,sans-serif;margin:2rem;background:#0f172a;color:#e2e8f0}
h1{color:#38bdf8}.summary{display:flex;gap:1rem;margin:1rem 0}
...
th,td{border:1px solid #334155;padding:0.5rem;font-size:0.9rem}
th{background:#1e293b}tr.status-fail{background:#450a0a}tr.status-pass{background:#052e16}
</style></head><body>
<h1>VSP Phone QA Report</h1>
<p>Generated: ${escapeHtml(report.generatedAt)} | API: ${escapeHtml(report.apiBase)} | Web: ${escapeHtml(report.webBase)}${report.gitCommit ? ` | ${escapeHtml(report.gitCommit)}` : ''}</p>
<div class="summary">
...
<table><thead><tr><th>Suite</th><th>Case</th><th>Status</th><th>Duration</th><th>Detail</th><th>Screenshot</th></tr></thead>
...
module.exports = { buildReport, writeHtmlReport };
