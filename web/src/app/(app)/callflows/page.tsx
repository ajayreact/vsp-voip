...
import { createV3CallFlow, deleteV3CallFlow, getV3CallFlows, type CallFlow } from '@/lib/v3-api';
...
export default function CallFlowsPage() {
  ...
  async function onCreate() {
    ...
    try {
      ...
      router.push(`/callflows/builder?id=${res.callFlow.id}`);
    } catch (err) {
            ...
            {items.map((flow) => (
              <tr key={flow.id}>
                ...
                <td className="px-4 py-3 space-x-3">
                  <Link href={`/callflows/builder?id=${flow.id}`} className="text-indigo-600 hover:underline">Builder</Link>
                  <Link href={`/callflows/simulator?id=${flow.id}`} className="text-indigo-600 hover:underline">Simulate</Link>
