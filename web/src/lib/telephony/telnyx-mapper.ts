...
function canConfirmFromSession(call: Call, session: CallSessionContext): boolean {
  const state = normalizeSdkCallState(call.state);
  if (state !== 'active' && state !== 'held') return false;
  if (session.remoteRingSeen) return true;
...
export function shouldConfirmRemoteAnswer(input: {
  ...
  const { call, session, eventType } = input;

  if (session.direction === 'inbound') {
    const state = normalizeSdkCallState(call.state);
    if (state === 'active' || state === 'held') {
      return { confirmed: true, source: eventType ? `inbound_${eventType}` : 'inbound_active' };
    }
  ...
  if (session.awaitingDeskBridge) {
    ...
    if (session.activeTransitionCount >= 1 && canConfirmFromSession(call, session)) {
      return { confirmed: true, source: 'internal_bridge_pstn_gate' };
    }
    return { confirmed: false, source: 'internal_bridge_deferred' };
  }

  if (canConfirmFromSession(call, session)) {
