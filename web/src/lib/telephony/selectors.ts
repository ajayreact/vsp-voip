...
export function selectDurationSeconds(snapshot: TelephonySnapshot, now = Date.now()): number {
  if (!CONNECTED_PHASES.has(snapshot.callPhase)) {
    return snapshot.session?.durationSeconds ?? 0;
  }
  const connectedAt = snapshot.session?.connectedAt;
