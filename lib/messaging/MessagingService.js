...
async function sendMessage({
  ...
  if (attachments.length) {
    payload.media_urls = attachments.map((item) => item.publicUrl).filter(Boolean);
    payload.type = 'MMS';
  }
...
async function saveInboundFromWebhook(prisma, {
  ...
  if (Array.isArray(mediaUrls) && mediaUrls.length) {
    const attachmentRows = mediaUrls.map((url) => ({
      tenantId,
      messageId: message.id,
      storageKey: `inbound-${telnyxMessageId || message.id}-${Buffer.from(url).toString('base64url').slice(0, 24)}`,
      publicUrl: url,
      mimeType: 'application/octet-stream',
      sizeBytes: 0,
    }));
    await prisma.messageAttachment.createMany({ data: attachmentRows });
    message = await prisma.message.findUnique({
      where: { id: message.id },
      include: { attachments: true },
    });
