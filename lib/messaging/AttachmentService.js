...
const { isSignedAttachmentUrl, isLegacyPublicAttachmentUrl } = require('./attachmentAccess');
...
function getApiPublicBase() {
  return require('./apiPublicBase').getApiPublicBase();
}

function buildPublicUrl(_storageKey) {
  return null;
}

function isDurablePublicUrl(publicUrl) {
  return isSignedAttachmentUrl(publicUrl) || isLegacyPublicAttachmentUrl(publicUrl);
}
...
async function getAttachmentForDownload(prisma, attachmentId) {
