...
async function countMissingRuntimeLinks(prisma, tenantId) {
  const [links, ...domainRows] = await Promise.all([
    prisma.v3RuntimeLink.findMany({
      where: { tenantId },
      select: { v3EntityType: true, v3EntityId: true },
    }),
    ...RUNTIME_ENTITY_MAP.map((domain) => domain.load(prisma, tenantId)),
  ]);

  const linkSet = new Set(links.map((l) => `${l.v3EntityType}:${l.v3EntityId}`));
  const byDomain = {};
  let total = 0;

  RUNTIME_ENTITY_MAP.forEach((domain, index) => {
    const rows = domainRows[index];
    let missing = 0;
    for (const row of rows) {
      if (!linkSet.has(`${domain.entityType}:${row.id}`)) missing += 1;
    }
    byDomain[domain.key] = missing;
    total += missing;
  });

  if (!linkSet.has(`holiday:${tenantId}`)) {
    byDomain.holidays = 1;
    total += 1;
  } else {
    byDomain.holidays = 0;
...
async function validate(prisma, tenantId) {
  ...
  const publishedFlows = callFlows.filter((f) => f.status === 'PUBLISHED');
  const flowValidations = await Promise.all(
    publishedFlows.map(async (flow) => ({
      flow,
      validation: await callFlowValidationService.validateFlow(prisma, tenantId, flow.definition),
    })),
  );
  for (const { flow, validation: flowValidation } of flowValidations) {
...
async function runMigration(prisma, tenantId, { req, dryRun = false, autoRollback = true } = {}) {
  ...
  try {
    ...
    let rollbackResult = null;
    if (!dryRun && autoRollback && backup?.id) {
      try {
        rollbackResult = await rollbackService.rollbackExecute(prisma, tenantId, {
          backupId: backup.id,
          dryRun: false,
        ...
        steps.push({ step: 'auto_rollback_on_error', ok: true, backupId: backup.id, error: error.message });
      } catch (rollbackError) {
        steps.push({
          step: 'auto_rollback_on_error',
          ok: false,
          backupId: backup.id,
          error: error.message,
    ...
    run = await prisma.v3MigrationRun.update({
      ...
      data: {
        ...
        backupId: backup?.id || null,
        rollbackBackupId: rollbackResult ? backup?.id : null,
        result: { error: error.message, steps, rollback: rollbackResult },
        finishedAt: new Date(),
      },
    }).catch(() => run);
