...
function pickExtensionIds(extIds, start, maxCount, { min = 1 } = {}) {
  const picked = extIds.slice(start, start + maxCount);
  if (picked.length >= min) return picked;
  return extIds.slice(0, Math.min(maxCount, extIds.length));
}
...
async function seedTestTenant(prisma, tenantId, options = {}, ctx = {}) {
  ...
  await runStep(steps, 'create_ring_groups', async () => {
    ...
    const group2 = await ringGroupService.createRingGroup(prisma, tenantId, {
      ...
      memberExtensionIds: pickExtensionIds(extIds, 5, 5),
  ...
  await runStep(steps, 'create_queues', async () => {
    ...
    const queue2 = await queueService.createQueue(prisma, tenantId, {
      ...
      agentExtensionIds: pickExtensionIds(extIds, 4, 4),
    }, { req: ctx.req, actor });
