...
function buildProvisionContext({
  ...
  return {
    ...
    sip: {
      ...
      // Port must follow the actual transport — portTls only applies when transport is TLS.
      // Previously this always preferred portTls, so UDP/TCP profiles still got the TLS port.
      port: (sip.transport === 'UDP' || sip.transport === 'TCP') ? sip.port : (sip.portTls || sip.port),
    ...
    extensionDialTargets: extensionDialTargets.map((ext) => ({
      extensionNumber: ext.extensionNumber,
      displayName: ext.displayName || ext.extensionNumber,
      did: ext.did,
    })),
    dedicatedDeskCredentialPilot: Boolean(dedicatedDeskCredentialPilot),
    configExport,
  };
}
...
function buildProvisionUrl(device, vendor) {
  ...
  if (id === 'grandstream') {
    return buildGrandstreamConfigServerPath();
  }
