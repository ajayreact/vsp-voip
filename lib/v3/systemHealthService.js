...
async function getSystemHealth(prisma, tenantId) {
  ...
  const domains = {
    ...
    license: { level: lifecycleHealth.license.level, score: lifecycleHealth.license.level === 'green' ? 100 : (lifecycleHealth.license.level === 'yellow' ? 70 : 40) },
    storage: { level: lifecycleHealth.storage.level, score: lifecycleHealth.storage.level === 'green' ? 100 : 75 },
    backup: { level: lifecycleHealth.backup.level, score: lifecycleHealth.backup.level === 'green' ? 100 : 60 },
    subscription: { level: lifecycleHealth.subscription.level, score: lifecycleHealth.subscription.level === 'green' ? 100 : (lifecycleHealth.subscription.level === 'yellow' ? 70 : 30) },
    billing: { level: lifecycleHealth.billing.level, score: lifecycleHealth.billing.level === 'green' ? 100 : (lifecycleHealth.billing.level === 'yellow' ? 65 : 20) },
  };
