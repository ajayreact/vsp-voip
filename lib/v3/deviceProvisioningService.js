...
async function generateDeviceConfig(prisma, tenantId, deviceId, { regenerate = false } = {}) {
  ...
  const provisionUrl = deviceTemplateService.buildProvisionUrl(deviceId, device.vendor);

  return { device, context, config, provisionUrl, extension, user: refreshedUser };
}
