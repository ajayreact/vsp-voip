...
async function loadAssignmentContext(prisma, tenantId, device) {
  ...
  let extensionDialTargets = [];
  if (isDedicatedDeskCredentialPilotTenant(tenantId)) {
    const siblings = await prisma.extension.findMany({
      where: { tenantId, status: 'ACTIVE', id: { not: extension.id } },
      select: {
        extensionNumber: true,
        displayName: true,
        primaryPhoneNumber: { select: { number: true, isActive: true } },
      },
      orderBy: { extensionNumber: 'asc' },
    });
    extensionDialTargets = siblings
      .filter((ext) => ext.primaryPhoneNumber?.number && ext.primaryPhoneNumber.isActive !== false)
      .map((ext) => ({
        extensionNumber: ext.extensionNumber,
        displayName: ext.displayName,
        did: ext.primaryPhoneNumber.number,
      }));
  }

  return { tenant, extension, user, phoneNumber, blfExtensions, extensionDialTargets };
}
...
async function generateDeviceConfig(prisma, tenantId, deviceId, { regenerate = false } = {}) {
  ...
  const { tenant, extension, user, phoneNumber, blfExtensions, extensionDialTargets } = await loadAssignmentContext(prisma, tenantId, device);
  ...
  const context = deviceTemplateService.buildProvisionContext({
    ...
    dedicatedDeskCredentialPilot: isDedicatedDeskCredentialPilotTenant(tenantId),
...
async function provisionDevice(prisma, tenantId, deviceId, { regenerate = false } = {}, { req, actor } = {}) {
  ...
  // "Regenerate" must reset everything, including the provisioning access
  // key — otherwise a leaked/rotated key would keep working after a
  // deliberate regenerate. Plain "Provision" (first-time or idempotent)
  // reuses an existing key so previously-configured phones keep working.
  const provisionKey = regenerate ? randomBytes(16).toString('hex') : (existingMeta.provisionKey || randomBytes(16).toString('hex'));
