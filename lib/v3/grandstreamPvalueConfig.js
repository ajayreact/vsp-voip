...
/**
 * Escape Grandstream dial-plan replace-rule brackets for XML provisioning.
 * Replace rules use <ext=DID> syntax; raw angle brackets break the phone's XML
 * parser and the rule is silently dropped (only the default x+ / *x+ tail applies).
 * @param {string} dialPlan
 */
function escapeGrandstreamDialPlanForXml(dialPlan) {
  return String(dialPlan || '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
}
...
function buildGrandstreamPvalueXml(ctx) {
  ...
  const body = pairs
    ...
    .join('\n')
    + (dialPlan
      // Replace-rule brackets must be XML-entity escaped (&lt; &gt;) so the phone's
      // parser delivers literal <ext=DID> to the dial-plan engine after decode.
      ? `\n    <P290>${escapeGrandstreamDialPlanForXml(dialPlan)}</P290>`
      : '');
