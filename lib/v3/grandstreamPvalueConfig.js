...
/**
 * Wrap dial plan for XML provisioning. Replace rules use <ext=DID> syntax.
 * - Raw < > break the XML parser (rule silently dropped).
 * - &lt; &gt; entities may be kept literally by GRP260x firmware (blocks all dials).
 * CDATA preserves literal angle brackets through XML parsing.
 * @param {string} dialPlan
 */
function wrapGrandstreamDialPlanForXml(dialPlan) {
  // Grandstream gs_provision expects the P-value element body; CDATA is the
  // standard way to embed replace-rule brackets without entity ambiguity.
  return `<![CDATA[${String(dialPlan || '')}]]>`;
}
...
// Grandstream factory default (Account 1 → Call Settings → Dial Plan). Preserved
// as the fallback tail so PSTN (+E.164), star codes, and multi-digit dials keep
// working after we prepend extension→DID replace rules for pilot tenants.
const DEFAULT_GRANDSTREAM_DIAL_PLAN_TAIL = 'x+ | \\+x+ | *x+ | *xx*x+';

/**
 * Escape a leading "+" for Grandstream dial-plan replacement values. The phone's
 * dial-plan grammar treats "+" as special unless escaped with a backslash.
 * @param {string} e164
 */
function escapeGrandstreamDialPlanE164(e164) {
...
/**
 * Build Account 1 dial plan (P290) for dedicated-credential pilot tenants.
 *
 * Runtime evidence (2026-07-08 PCAP, GRP2601 fw 1.0.7.11): desk phones that
 * register via a Telnyx SIP credential send INVITE sip:101@sip.telnyx.com
 * directly to Telnyx's edge. Telnyx rejects short codes with
 * "404 Invalid caller ID number for emergency services D8" (UNALLOCATED_NUMBER)
 * before our Call Control Application ever sees the call. Desk→desk via the
 * extension's full DID already works because Telnyx accepts the E.164 destination
 * and forwards it to our inbound handler.
 *
 * Fix: prepend Grandstream replace rules (<ext=DID>) so a short extension dial
 * is rewritten to the callee's DID locally on the phone before the INVITE is sent.
 * Pilot-gated via ctx.dedicatedDeskCredentialPilot (see deskDedicatedCredentialPilot.js).
 *
 * @param {object} ctx
 * @param {boolean} [ctx.dedicatedDeskCredentialPilot]
 * @param {Array<{extensionNumber: string, did: string}>} [ctx.extensionDialTargets]
 * @returns {string|null} P290 value, or null when unchanged
 */
function buildGrandstreamDialPlan(ctx) {
  if (!ctx?.dedicatedDeskCredentialPilot) return null;
  const targets = Array.isArray(ctx.extensionDialTargets) ? ctx.extensionDialTargets : [];
  const replaceRules = targets
    .filter((t) => t?.extensionNumber && t?.did)
    .map((t) => `<${String(t.extensionNumber).trim()}=${escapeGrandstreamDialPlanE164(t.did)}>`);
  if (!replaceRules.length) return null;
  return `{ ${replaceRules.join(' | ')} | ${DEFAULT_GRANDSTREAM_DIAL_PLAN_TAIL} }`;
}
...
function buildGrandstreamPvalueXml(ctx) {
  ...
  const dialPlan = buildGrandstreamDialPlan(ctx);
  ...
  const body = pairs
    ...
    .join('\n')
    + (dialPlan
      ? `\n    <P290>${wrapGrandstreamDialPlanForXml(dialPlan)}</P290>`
      : '');
