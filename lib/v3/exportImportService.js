...
function validateImportPayload(payload, tenantId) {
  const issues = [];
  if (!payload || typeof payload !== 'object') {
    issues.push({ severity: 'error', code: 'INVALID_PAYLOAD', message: 'Import payload must be an object' });
    return { valid: false, issues };
  }
  if (payload.tenantId && tenantId && String(payload.tenantId) !== String(tenantId)) {
    issues.push({
      severity: 'error',
      code: 'TENANT_MISMATCH',
      message: 'Import payload tenantId does not match target tenant',
    });
  }
...
async function validateImport(prisma, tenantId, payload) {
  const validation = validateImportPayload(payload, tenantId);
