...
async function createDevice(prisma, tenantId, data, { req, actor } = {}) {
  ...
  if (macAddress) {
    ...
    // Self-heal: (tenantId, macAddress) is a DB-level unique constraint, so a
    // legacy soft-deleted row (status=REMOVED, pre-dating the hard-delete fix
    // in removeDevice) would otherwise block re-adding the same physical
    // phone with a P2002 unique-constraint error. Purge it before inserting.
    const stale = await prisma.v3DeskDevice.findFirst({
      where: { tenantId, macAddress, status: 'REMOVED' },
    });
    if (stale) {
      await runtimeAdapter.removeLink(prisma, tenantId, 'device', stale.id);
      await prisma.v3DeskDevice.delete({ where: { id: stale.id } });
    }
  }
...
/**
 * Delete a device's provisioning registration.
 *
 * This is a hard delete of the V3DeskDevice row (device record, provisioning
 * profile/URL, config/provision version counters, and provisioning metadata
 * such as the provisionKey all live on this single row, so deleting it wipes
 * all of that state). It intentionally does NOT touch:
 *   - the Employee (User row)
 *   - the Extension row
 *   - SIP credentials (telnyxSipUsername/telnyxSipPassword on User)
 *   - Telnyx connection/configuration
 *
 * A prior soft-delete implementation (status='REMOVED') left the row in
 * place, which tripped the (tenantId, macAddress) unique constraint and
 * silently blocked re-adding the same physical phone. Hard-deleting frees
 * the MAC address immediately so "Add Device" with the same MAC always
 * regenerates a brand-new record with fresh config/provision versions.
 */
async function removeDevice(prisma, tenantId, deviceId, { req, actor } = {}) {
  ...
  await prisma.v3DeskDevice.delete({ where: { id: deviceId } });

  // Purge the runtime sync link (provisioning cache pointer) so nothing
  // references this device id after it no longer exists.
  await runtimeAdapter.removeLink(prisma, tenantId, 'device', deviceId);
  ...
  await auditService.log(prisma, req, {
    ...
    extra: {
      tenantId,
      actor: actor?.sub,
      note: 'Hard delete: employee, extension, and SIP credentials are unaffected',
    },
  });

  return { id: deviceId, deleted: true, ...snapshot };
}
