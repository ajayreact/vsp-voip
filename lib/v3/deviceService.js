...
async function removeDevice(prisma, tenantId, deviceId, { req, actor } = {}) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.v3RuntimeLink.deleteMany({
      where: { tenantId, v3EntityType: 'device', v3EntityId: deviceId },
    });
    await tx.v3DeskDevice.delete({ where: { id: deviceId } });
  });
  ...
  await enqueueRuntimeSync(prisma, tenantId, 'device', deviceId, 'delete', { req });

  return { removed: true, ...snapshot };
}

async function removeDeviceByMac(prisma, tenantId, macAddress, options = {}) {
  const mac = normalizeMac(macAddress);
  if (!mac) {
    throw Object.assign(new Error('MAC address is required'), { status: 400 });
  }

  const device = await prisma.v3DeskDevice.findFirst({
    where: { tenantId, macAddress: mac, status: { not: 'REMOVED' } },
  });
  if (!device) {
    throw Object.assign(new Error('Device not found for this MAC address'), { status: 404 });
  }

  return removeDevice(prisma, tenantId, device.id, options);
}
