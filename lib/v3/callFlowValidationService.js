...
async function validateReferences(prisma, tenantId, definition) {
  ...
  for (const node of def.nodes) {
    ...
    if (node.type === 'VOICEMAIL') {
      const boxId = node.data?.voicemailBoxId;
      const extId = node.data?.extensionId;
      if (boxId) {
        const box = await prisma.v3VoicemailBox.findFirst({ where: { id: boxId, tenantId, removedAt: null }, select: { id: true } });
        if (!box) results.push(issue('error', 'MISSING_VOICEMAIL', `Voicemail box not found: ${boxId}`, node.id));
      } else if (extId) {
        const ext = await prisma.extension.findFirst({ where: { id: extId, tenantId }, select: { id: true } });
        if (!ext) results.push(issue('error', 'INVALID_EXTENSION', `Extension not found: ${extId}`, node.id));
      } else {
        results.push(issue('error', 'MISSING_VOICEMAIL', 'Voicemail node missing voicemailBoxId or extensionId', node.id));
      }
    }
    ...
    if (node.type === 'RING_GROUP') {
      ...
      const v3rg = await prisma.v3RingGroup.findFirst({ where: { id: rgId, tenantId, removedAt: null }, select: { id: true } });
      if (!v3rg) {
        const legacy = await prisma.ringGroup.findFirst({ where: { id: rgId, tenantId }, select: { id: true } });
        if (!legacy) results.push(issue('error', 'MISSING_RING_GROUP', `Ring group not found: ${rgId}`, node.id));
      }
    }

    if (node.type === 'QUEUE') {
      const queueId = node.data?.queueId;
      if (!queueId && !node.data?.queueName) {
        results.push(issue('error', 'MISSING_QUEUE', 'Queue node missing queueId or queueName', node.id));
        continue;
      }
      if (queueId) {
        const q = await prisma.v3Queue.findFirst({ where: { id: queueId, tenantId, removedAt: null }, select: { id: true } });
        if (!q) results.push(issue('error', 'MISSING_QUEUE', `Queue not found: ${queueId}`, node.id));
      }
    }

    if (node.type === 'BUSINESS_HOURS' && node.data?.scheduleId) {
      const sched = await prisma.v3BusinessHoursSchedule.findFirst({
        where: { id: node.data.scheduleId, tenantId, removedAt: null },
        select: { id: true },
      });
      if (!sched) results.push(issue('error', 'MISSING_SCHEDULE', `Business hours schedule not found: ${node.data.scheduleId}`, node.id));
    }

    if (node.type === 'HOLIDAY' && node.data?.holidayId) {
      const hol = await prisma.v3Holiday.findFirst({
        where: { id: node.data.holidayId, tenantId, removedAt: null },
        select: { id: true },
      });
      if (!hol) results.push(issue('error', 'MISSING_HOLIDAY', `Holiday not found: ${node.data.holidayId}`, node.id));
    }
