...
async function userUxHealth(prisma, tenantId, userId) {
  const [profile, devices, presence, directory] = await Promise.all([
    softphoneProfileService.getOrCreateProfile(prisma, tenantId, userId),
    devicePreferenceService.listDevicePreferences(prisma, tenantId, userId),
    presenceService.getPresence(prisma, tenantId, userId),
    contactDirectoryService.directorySyncHealth(prisma, tenantId),
  ]);
...
async function softphoneUxHealth(prisma, tenantId) {
  ...
  const userHealth = await Promise.all(users.map((u) => userUxHealth(prisma, tenantId, u.id)));
