...
async function assignNumberToTenant(prisma, {
  ...
  await enqueueRuntimeSync(prisma, tenantId, 'number', phoneNumberId, 'sync', { req });
...
async function assignNumberToExtension(prisma, tenantId, {
  ...
  await enqueueRuntimeSync(prisma, tenantId, 'number', result.phone.id, 'sync', { req });
  if (result.extension?.id) {
    await enqueueRuntimeSync(prisma, tenantId, 'extension', result.extension.id, 'sync', { req });
  }
