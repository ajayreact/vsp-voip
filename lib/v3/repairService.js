...
async function analyzeNumbers(prisma, tenantId) {
  ...
  for (const phone of phoneNumbers) {
    const ext = phone.extension;

    if (phone.tenantId && phone.extensionId && !ext) {
      changes.push(makeChange({
        type: 'BROKEN_EXTENSION_LINK',
        entity: 'PhoneNumber',
        ref: phone.number,
        detail: 'extensionId points to missing extension — clear link',
        write: () => prisma.phoneNumber.update({
          where: { id: phone.id },
    ...
    if (ext && ext.primaryPhoneNumberId !== phone.id && phone.extensionId === ext.id) {
      changes.push(makeChange({
        type: 'MISSING_PRIMARY_DID',
        entity: 'Extension',
        ref: ext.extensionNumber,
        detail: `set primaryPhoneNumberId to ${phone.number}`,
        write: () => prisma.extension.update({
          where: { id: ext.id },
    ...
    if (ext && ext.userId && phone.assignedUserId !== ext.userId) {
      changes.push(makeChange({
        type: 'BROKEN_EMPLOYEE_LINK',
        entity: 'PhoneNumber',
        ref: phone.number,
        detail: 'align assignedUserId to extension owner',
        write: () => prisma.phoneNumber.update({
          where: { id: phone.id },
          data: { assignedUserId: ext.userId },
        }),
      }));
    }

    if (ext && !phone.extensionId) {
      changes.push(makeChange({
        type: 'MISSING_EXTENSION_LINK',
        entity: 'PhoneNumber',
        ref: phone.number,
        detail: `back-link to extension ${ext.extensionNumber}`,
        write: () => prisma.phoneNumber.update({
          where: { id: phone.id },
    ...
    const effectiveUserId = phone.assignedUserId || ext?.userId || null;
    if (phone.tenantId && phone.routingType === 'tenant_default' && phone.extensionId && effectiveUserId) {
      changes.push(makeChange({
        type: 'WRONG_ROUTING',
        entity: 'PhoneNumber',
        ref: phone.number,
        detail: 'set routingType to direct_user for assigned extension DID',
        write: () => prisma.phoneNumber.update({
          where: { id: phone.id },
          data: { routingType: 'direct_user' },
        }),
      }));
    }

    if (phone.tenantId && !phone.extensionId && !phone.assignedUserId) {
      observations.push(`Number ${phone.number}: assigned to tenant but not linked to employee or extension.`);
    }
  }
  ...
  return {
    changes,
    observations,
    scanned: { phoneNumbers: phoneNumbers.length },
  };
}

async function inspectNumbers(prisma, tenantId = null) {
  const result = await analyzeNumbers(prisma, tenantId);
  return {
    mode: 'dry-run',
    scanned: result.scanned,
    changes: result.changes.map(publicChange),
...
async function repairNumbers(prisma, tenantId = null, { apply = false } = {}) {
  const result = await analyzeNumbers(prisma, tenantId);
  if (!apply) {
    return {
      mode: 'dry-run',
...
module.exports = { inspect, repair, analyze, inspectNumbers, repairNumbers, analyzeNumbers };
