...
/**
 * Purge a stale runtime link when the underlying V3 entity is deleted
 * (e.g. desk device removal) so no cached sync metadata survives the entity.
 * Safe to call even when no link exists.
 */
async function removeLink(prisma, tenantId, v3EntityType, v3EntityId) {
  await prisma.v3RuntimeLink.deleteMany({
    where: { tenantId, v3EntityType, v3EntityId },
  });
}
