...
async function reassignExtensionEmployee(prisma, tenantId, extensionId, body, actor = {}) {
  ...
  if (updatedExtension) {
    ...
    // Provision employee telephony (Telnyx SIP credential) immediately after assignment,
    // using the same implementation as extension creation. Idempotent: reuses an existing
    // valid credential and only creates one when User.telnyxCredentialId is missing/invalid.
    // Non-fatal so a Telnyx outage cannot break the assignment (lazy repair still applies).
    const hadCredentialBefore = Boolean(newUser.telnyxCredentialId);
    console.log('[extensionOwnership] Employee assigned', {
      tenantId,
      extensionId,
      extensionNumber: updatedExtension.extensionNumber,
      userId: newUser.id,
    });
    try {
      console.log('[extensionOwnership] ensureEmployeeTelephonyForExtension called', {
        extensionId,
        userId: newUser.id,
        hadCredentialBefore,
      });
      await ensureEmployeeTelephonyForExtension(prisma, updatedExtension);
      const provisionedUser = await prisma.user.findUnique({
        where: { id: newUser.id },
        select: { telnyxCredentialId: true, telnyxSipUsername: true },
      });
      const created = !hadCredentialBefore && Boolean(provisionedUser?.telnyxCredentialId);
      console.log(
        created
          ? '[extensionOwnership] Credential created'
          : '[extensionOwnership] Credential already existed',
        {
          extensionId,
          userId: newUser.id,
          telnyxCredentialId: provisionedUser?.telnyxCredentialId || null,
          telnyxSipUsername: provisionedUser?.telnyxSipUsername || null,
        },
      );
    } catch (error) {
      console.warn(
        `[extensionOwnership] employee Telnyx credential not provisioned on assignment: ${error.message}`,
      );
    }
  }
