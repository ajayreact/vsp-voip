...
async function resetExtensionSipCredentials(prisma, tenantId, extensionId, actor = {}) {
  ...
  const previousCredentialId = extension.telnyxCredentialId;

  let updated;
  try {
    updated = await ensureExtensionTelnyxCredential(prisma, extension, { forceRecreate: true });
  } catch (error) {
    throw Object.assign(
      new Error(error.message || 'Failed to reset extension desk SIP credentials'),
  ...
  await writeExtensionAuditLog(prisma, {
    ...
    summary: `Desk SIP credentials reset for extension ${extension.extensionNumber}`,
    changes: {
      previousCredentialId,
      newCredentialId: updated?.telnyxCredentialId || null,
      telnyxSipUsername: updated?.telnyxSipUsername || null,
    },
  });

  return {
    reset: true,
    sipUsername: updated?.telnyxSipUsername || extension.extensionNumber,
    sipPassword: updated?.telnyxSipPassword || null,
    previousCredentialId,
    newCredential: updated?.telnyxCredentialId
      ? {
        credentialId: updated.telnyxCredentialId,
        sipUsername: updated.telnyxSipUsername,
        sipPassword: updated.telnyxSipPassword || null,
      }
      : null,
  };
