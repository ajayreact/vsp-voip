...
async function setExtensionPrimaryDid(prisma, tenantId, extensionId, phoneNumberId, actor = {}, options = {}) {
  ...
  // Guarantee employee telephony provisioning after a DID is assigned.
  const { ensureExtensionTelephonyProvisioned } = require('./pbxRebuild');
  await ensureExtensionTelephonyProvisioned(prisma, extension, { stage: 'assign-did' });
...
async function reassignExtensionEmployee(prisma, tenantId, extensionId, body, actor = {}) {
  ...
  if (updatedExtension) {
    ...
    // Guarantee employee telephony provisioning immediately after assignment via the
    // shared, idempotent orchestration helper (reuses existing credential, creates
    // only when missing, never throws).
    const { ensureExtensionTelephonyProvisioned } = require('./pbxRebuild');
    await ensureExtensionTelephonyProvisioned(prisma, updatedExtension, { stage: 'assign-employee' });
  }
...
async function resetExtensionSipCredentials(prisma, tenantId, extensionId, actor = {}) {
  ...
  const { ensureExtensionTelephonyProvisioned } = require('./pbxRebuild');
  await ensureExtensionTelephonyProvisioned(prisma, extension, { stage: 'generate-sip-credentials' });
