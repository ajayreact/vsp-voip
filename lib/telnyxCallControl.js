...
async function telnyxApiRequest(method, path, data) {
  ...
  const isDialPath = method === 'post' && (
    path === '/calls'
    || /\/actions\/dial$/.test(path)
  );
  if (isDialPath) {
    try {
      const { traceDeskDesk } = require('./telephony/deskDeskTrace');
      traceDeskDesk('Dial Request', {
        method,
        path,
        destination: data?.to ?? null,
        dial: data?.to ?? null,
        callControlId: data?.call_control_id ?? null,
  ...
  try {
    ...
    if (isDialPath) {
      try {
        const { traceDeskDesk } = require('./telephony/deskDeskTrace');
        traceDeskDesk('Dial Response', {
          method,
          path,
          status: response.status ?? null,
          ok: true,
          callControlId: result?.call_control_id ?? data?.call_control_id ?? null,
        });
      } catch {
        // trace optional
      }
    ...
    if (isDialPath) {
      try {
        ...
        traceDeskDesk('Dial Response', {
          ...
          callControlId: error.response?.data?.data?.call_control_id ?? data?.call_control_id ?? null,
        });
      } catch {
        // trace optional
      }
...
async function dialDestination(callControlId, {
  ...
  timeoutSecs = 25,
  clientState,
  // Ring-first (pilot-scoped) dials the desk phone with bridge_on_answer OMITTED
  // (Telnyx defaults to false) because the linked PSTN leg is intentionally left
  // parked/unanswered; bridging is done manually via bridgeCalls() once the desk
  // leg answers (see bridgeParkedInboundToAgentLeg). Every existing call site
  // relies on the answered-inbound-leg + bridge_on_answer:true behavior and is
  // unaffected since this defaults to true.
  bridgeOnAnswer = true,
}) {
  ...
  const result = await telnyxApiRequest('post', '/calls', {
    to,
    from,
...
async function holdCall(callControlId, clientState) {
  const body = clientState ? { client_state: clientState } : {};
  return callControlAction(callControlId, 'hold', body);
}

async function unholdCall(callControlId, clientState) {
  const body = clientState ? { client_state: clientState } : {};
  return callControlAction(callControlId, 'unhold', body);
}

async function joinConference(callControlId, {
  conferenceName,
  conferenceId,
  startConferenceOnEnter = false,
  clientState,
}) {
  const name = conferenceName || conferenceId;
  if (!name) {
    throw Object.assign(new Error('conferenceName or conferenceId is required'), { status: 400 });
  }
  return callControlAction(callControlId, 'join_conference', {
    conference_name: String(name).slice(0, 128),
    start_conference_on_enter: Boolean(startConferenceOnEnter),
    ...(clientState ? { client_state: clientState } : {}),
  });
}

async function leaveConference(callControlId, clientState) {
  const body = clientState ? { client_state: clientState } : {};
  return callControlAction(callControlId, 'leave_conference', body);
}

async function muteCall(callControlId, clientState) {
  const body = clientState ? { client_state: clientState } : {};
  return callControlAction(callControlId, 'mute', body);
}

async function unmuteCall(callControlId, clientState) {
  const body = clientState ? { client_state: clientState } : {};
  return callControlAction(callControlId, 'unmute', body);
}
