...
/**
 * Production pilot overrides — pinned to exact extension rows or tenant+number.
...
const TEMP_DESK_FIRST_OVERRIDES = Object.freeze([
  // Local/dev Asuitech ext 101
  {
    extensionId: 'dda183d4-7cd0-43dc-b395-a0622061dc76',
  },
  // Production VSP Internal ext 101 (+19563961388)
  {
...
function getDeskFirstOverrides() {
  const overrides = [...TEMP_DESK_FIRST_OVERRIDES];
  const envIds = process.env.RING_DESK_FIRST_PILOT_EXTENSION_IDS || '';
  for (const extensionId of envIds.split(',').map((item) => item.trim()).filter(Boolean)) {
    overrides.push({ extensionId });
  }
  return overrides;
}
...
function matchesDeskFirstOverride(extension) {
  ...
  return getDeskFirstOverrides().some((override) => {
