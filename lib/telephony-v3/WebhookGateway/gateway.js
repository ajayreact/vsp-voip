...
async function handleV3WebhookIngress(body, meta = {}) {
  ...
  traceDeskDesk('gateway.handleV3WebhookIngress()', {
    event_type: body?.data?.event_type ?? null,
    callControlId: body?.data?.payload?.call_control_id ?? null,
    connectionId: body?.data?.payload?.connection_id ?? null,
    from: body?.data?.payload?.from ?? null,
    to: body?.data?.payload?.to ?? null,
  ...
  return withSpan('telephony.webhook.receive', { source: meta.source || 'unknown', traceId }, async () => {
    ...
    traceDeskDesk('Webhook Received', {
      ...
      callControlId: normalized.callControlId || null,
      connectionId: body?.data?.payload?.connection_id ?? null,
      from: body?.data?.payload?.from ?? null,
      to: body?.data?.payload?.to ?? null,
      direction: body?.data?.payload?.direction ?? null,
      source: meta.source || null,
    });
