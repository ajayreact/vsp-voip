...
const { logDialBridge, resolveDestination } = require('./dialBridgeDiagnostics');
  ...
  try {
    switch (commandType) {
      ...
      case 'BRIDGE': {
        ...
        logDialBridge('request', {
          commandType: 'BRIDGE',
          destination: otherCallControlId ? String(otherCallControlId) : 'pending_target_leg',
          callControlId,
          targetLegId: input.targetLegId ?? null,
          commandId: input.commandId,
          telnyxEndpoint: `POST ${bridgePath}`,
          requestPayload: bridgeBody,
        });
        if (!otherCallControlId) {
          ...
          logDialBridge('validation_failed', {
            commandType: 'BRIDGE',
            destination: payload.pendingTargetLeg ? 'pending_target_leg' : null,
            callControlId,
            targetLegId: input.targetLegId ?? null,
            commandId: input.commandId,
            telnyxEndpoint: `POST ${bridgePath}`,
            requestPayload: bridgeBody,
            skipReason,
            exceptionMessage: 'otherCallControlId is required for BRIDGE',
          });
      ...
      case 'DIAL': {
        ...
        logDialBridge('request', {
          ...
          targetLegId: input.targetLegId ?? null,
          commandId: input.commandId,
          telnyxEndpoint: 'POST /calls',
          requestPayload: dialBody,
        });
        if (!to) {
          logDialBridge('validation_failed', {
            commandType: 'DIAL',
            ...
            exceptionMessage: 'to is required for DIAL',
          });
    ...
    if (commandType === 'DIAL' || commandType === 'BRIDGE') {
      const endpoint = commandType === 'DIAL'
        ? 'POST /calls'
        : `POST /calls/${encodeURIComponent(callControlId)}/actions/bridge`;
      logDialBridge('error', {
        commandType,
        destination: resolveDestination(commandType, payload),
        callControlId,
        targetLegId: input.targetLegId ?? null,
        commandId: input.commandId,
        telnyxEndpoint: endpoint,
        requestPayload: payload,
        responseStatus: normalized.status ?? null,
        responseBody: normalized.telnyx,
