...
const { isDialBridgeCommand, logDialBridge, resolveDestination } = require('./dialBridgeDiagnostics');
...
async function executeOneCommandLocked(row, workerId, options = {}) {
  ...
  const commandPayload = /** @type {Record<string, unknown>} */ (row.payload || {});
  if (isDialBridgeCommand(row.commandType)) {
    logDialBridge('executor_start', {
      commandType: row.commandType,
      commandId: row.id,
      sessionId: row.sessionId,
      targetLegId: row.legId ?? null,
      callControlId: context.callControlId,
      destination: resolveDestination(row.commandType, commandPayload),
  ...
  try {
    ...
    if (adapterResult.skipped) {
      if (isDialBridgeCommand(row.commandType)) {
        logDialBridge('executor_skipped', {
          commandType: row.commandType,
          commandId: row.id,
          sessionId: row.sessionId,
          targetLegId: row.legId ?? null,
      ...
      if (isDialBridgeCommand(row.commandType)) {
        logDialBridge('executor_completed', {
          commandType: row.commandType,
          commandId: row.id,
          sessionId: row.sessionId,
          targetLegId: row.legId ?? null,
    ...
    if (isDialBridgeCommand(row.commandType)) {
      logDialBridge('executor_failed', {
        ...
        skipReason: failedRow?.status === 'DEAD' ? 'dead_letter' : null,
      });
    }
