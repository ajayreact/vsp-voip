...
/**
 * Resolves the tenant's telephony provider (defaults to Telnyx for every
 * tenant without an explicit TenantProvider row) and dispatches the command
 * through the provider's generic `executeCommand()`, translating the result
 * back into the shape the rest of this file already expects
 * (`telnyxRequestId`/`telnyxResult` field names are kept for compatibility
 * with `V3CommandOutbox.telnyxRequestId` and existing consumers).
 * @param {import('@prisma/client').V3CommandOutbox} row
 * @param {Record<string, unknown>} context
 */
async function executeViaResolvedProvider(row, context) {
  const provider = await ProviderResolver.resolve(context.tenantId);
  ...
  return {
    ...
    telnyxRequestId: result.externalRequestId ?? null,
    telnyxResult: result.providerResult ?? null,
  };
}
...
async function executeOneCommandLocked(row, workerId, options = {}) {
  ...
  try {
    // `options.telnyx` is a direct dependency-injection seam used only by
    // tests (and any other explicit caller) to exercise telnyxAdapter with a
    // mocked Telnyx client — when present, bypass provider resolution and
    // call the adapter directly so that existing test/behavior contracts are
    // unchanged. In all other cases (production), resolve the tenant's
    // provider via ProviderResolver so non-Telnyx tenants can be routed to
    // e.g. TwilioProvider without any other change to this file.
    const adapterResult = options.telnyx
      ...
      : await executeViaResolvedProvider(row, context);
