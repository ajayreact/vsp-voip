...
function logCommandExecutorFailure(row, context, error, extra = {}) {
  ...
  const logLabel = extra.outcome === 'idempotent_success'
    ? '[V3] commandExecutor idempotent'
    : '[V3] commandExecutor failed';
  console.log(logLabel, record);
  v3Logger.error('command.executor.failed', record);
}
