...
const { isTelnyxV3CallControlId, isOutboundDirection } = require('../../telephony/PayloadNormalizer');
...
function inferSessionOrigin(normalized) {
  ...
  if (isOutboundDirection(normalized.direction) && isTelnyxV3CallControlId(normalized.callControlId)) {
    return 'DESK';
  }
...
async function bootstrapSessionAndLeg(
  ...
  return locks.withBootstrapLock(
    ...
    async () => {
      ...
      traceDeskDesk('bootstrapSessionAndLeg()', {
        tenantId,
        callControlId: normalized.callControlId,
        connectionId: normalized.connectionId ?? null,
        sessionId: loadedSession?.id ?? null,
        origin: loadedSession?.origin ?? sessionInput.origin ?? null,
        targetExtension: normalized.to ?? null,
        extensionNumber: tenantResolution?.extensionNumber ?? null,
        createdSession,
        createdLeg,
        eventId,
...
async function processIngressEvent(ctx) {
  ...
  let session = leg ? await sessionManager.loadSession(leg.sessionId, null) : null;

  let tenantResolution = await resolveTenantForWebhook(normalized);
  if (!tenantResolution.tenantId && session?.tenantId) {
    tenantResolution = {
      tenantId: session.tenantId,
      source: 'session_existing',
    };
  }
  if (tenantResolution.rejected && !session) {
  ...
  const tenantId = tenantResolution.tenantId ?? session?.tenantId ?? null;
  ...
  return withSpan('telephony.callmanager.process', {
    ...
    leg = leg || await legManager.findLegByCallControlId(normalized.callControlId);
    session = leg ? await sessionManager.loadSession(leg.sessionId, tenantId) : session;
    ...
    if (!session && isInboundBootstrap) {
      const origin = inferSessionOrigin(normalized);
      if (!tenantId && origin === 'PSTN_INBOUND') {
      ...
      if (!tenantId && origin === 'DESK') {
        metrics.tenantBootstrapFailed({ reason: 'missing_tenant_desk' });
        v3Logger.warn('callmanager.desk_bootstrap_rejected', {
          callControlId: normalized.callControlId,
          eventType: normalized.eventType,
          ingressId,
        });
        return { handled: false, rejected: true, reason: 'missing_tenant_desk' };
      }
