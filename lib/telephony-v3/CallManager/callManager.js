 ...
 */
async function bootstrapSessionAndLeg(normalized, tenantId, eventId, workerId, ingressId) {
  return locks.withBootstrapLock(
    ...
    async () => {
      ...
      const { session, created: createdSession } = await sessionManager.findOrCreateSession({
        tenantId,
