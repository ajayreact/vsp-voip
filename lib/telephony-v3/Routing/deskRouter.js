...
async function hasExistingRoute(sessionId) {
  ...
  const { isRoutedRouteSnapshot } = require('../Utils/routeSnapshotHelper');
  return isRoutedRouteSnapshot(row?.routeSnapshot);
}
...
async function routeDeskSession(event) {
  ...
  if (handledEvents.has(dedupeKey)) {
    v3Logger.info('desk.router.skipped', { sessionId: event.sessionId, skipReason: 'duplicate_event' });
  ...
  console.log('[V3] deskRouter starting', {
    sessionId: event.sessionId,
    tenantId: event.tenantId ?? null,
    eventType: event.eventType,
  });

  traceDeskDesk('desk.router.start()', {
    tenantId: event.tenantId ?? null,
    sessionId: event.sessionId,
    callControlId: event.callControlId ?? null,
    origin: event.payload?.origin ?? null,
    destination: event.payload?.toAddress ?? event.payload?.destination ?? null,
    eventType: event.eventType,
  });
  ...
  try {
    ...
    if (session.origin !== 'DESK') {
      v3Logger.info('desk.router.skipped', { sessionId: session.id, skipReason: 'not_desk_origin' });
      return { skipped: true, reason: 'not_desk_origin' };
    }

    if (await hasExistingRoute(session.id)) {
      v3Logger.info('desk.router.skipped', { sessionId: session.id, skipReason: 'already_routed' });
    ...
    if (!flags.deskEnabled) {
      v3Logger.info('desk.router.skipped', { sessionId: session.id, skipReason: 'desk_disabled' });
    ...
    console.log('[V3] deskRouter policyDecision', {
      sessionId: session.id,
      action: policyDecision.action,
      effectiveAction: policyDecision.effectiveAction,
      allowed: policyDecision.allowed,
      observeOnly: policyDecision.observeOnly,
    });
    ...
    console.log('[V3] deskCommandBuilder completed', {
      sessionId: session.id,
      tenantId: session.tenantId,
      routingFlow: destResult.routingFlow,
      commandCount: commands.length,
      commandTypes: commands.map((command) => command.commandType),
    });
    ...
    if (!flags.observeOnly && commands.length) {
      ...
      const enqueuedRows = await commandBus.enqueueIntents(commands, {
        sessionId: session.id,
        legId: originLeg.id,
        tenantId: session.tenantId,
      ...
      console.log('[V3] deskRouter commands enqueued', {
        sessionId: session.id,
        tenantId: session.tenantId,
        routingFlow: destResult.routingFlow,
        commandCount: enqueuedRows.length,
        commandIds: enqueuedRows.map((row) => row.id),
        commandTypes: enqueuedRows.map((row) => row.commandType),
      });
      v3Logger.info('desk.router.commands_enqueued', {
        sessionId: session.id,
        tenantId: session.tenantId,
        routingFlow: destResult.routingFlow,
        commandCount: enqueuedRows.length,
        commands: enqueuedRows.map((row) => ({ id: row.id, commandType: row.commandType })),
      });
    } else {
      const skipReason = flags.observeOnly
        ? 'observe_only_mode'
        : (commands.length ? 'unknown' : 'zero_commands');
      console.log('[V3] deskRouter commands NOT enqueued', {
        sessionId: session.id,
        tenantId: session.tenantId,
        routingFlow: destResult.routingFlow,
        skipReason,
      ...
      v3Logger.info('desk.router.commands_skipped', {
        sessionId: session.id,
        tenantId: session.tenantId,
        routingFlow: destResult.routingFlow,
        skipReason,
