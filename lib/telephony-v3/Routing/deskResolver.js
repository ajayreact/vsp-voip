...
async function resolveDeskDestination(tenantId, toAddress, caller) {
  ...
  if (destination.kind === 'EXTENSION' && destination.extensionNumber) {
    ...
    traceDeskDesk('deskResolver.resolveDeskDestination', {
      tenantId,
      targetExtension: destination.extensionNumber,
      targetCount: appTargets.length,
      routingFlowPreview: appTargets.some((t) => t.type === 'app') ? 'DESK_TO_MOBILE' : 'DESK_TO_DESK',
    });
