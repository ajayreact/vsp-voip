...
/**
 * Desk outbound tenant bootstrap — call.initiated with Telnyx v3: call_control_id
 * may omit state=parked while still being a desk parked outbound leg.
 * @param {import('../types').V3NormalizedWebhook} normalized
 */
function isDeskOutboundBootstrapWebhook(normalized) {
  if (isDeskOutboundWebhook(normalized)) {
    return true;
  }
  if (normalized.eventType !== 'call.initiated') {
    return false;
  }
  return isOutboundDirection(normalized.direction)
    && isTelnyxV3CallControlId(normalized.callControlId);
}
...
async function resolveDeskOutboundContext(normalized) {
  if (!isDeskOutboundBootstrapWebhook(normalized)) {
    return null;
  }
...
module.exports = {
  ...
  isDeskOutboundBootstrapWebhook,
  getTelnyxPayload,
