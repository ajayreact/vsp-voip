...
const { isOutboundDirection } = require('../../telephony/PayloadNormalizer');
...
/**
 * Resolve tenant from desk parked outbound SIP from (legacy CallerResolver).
 * @param {import('../types').V3NormalizedWebhook} normalized
 */
async function resolveTenantFromDeskOutboundFrom(normalized) {
  const isDeskParkedOutbound = String(normalized.state || '').toLowerCase() === 'parked'
    && isOutboundDirection(normalized.direction);
  if (!isDeskParkedOutbound || !normalized.from) {
    return null;
  }

  const prisma = await getPrisma();
  const { resolveCallerFromAddress } = require('../../telephony/CallerResolver');
  const caller = await resolveCallerFromAddress(prisma, normalized.from);
  if (!caller?.tenantId) {
    return null;
  }
  return String(caller.tenantId);
}

/**
 * Resolve tenant for a webhook before session bootstrap.
 * Priority: client_state → inbound DID lookup → desk parked outbound SIP from.
 *
 * @param {import('../types').V3NormalizedWebhook} normalized
...
async function resolveTenantForWebhook(normalized) {
  ...
  const deskOutboundTenantId = await resolveTenantFromDeskOutboundFrom(normalized);
  if (deskOutboundTenantId) {
    metrics.tenantBootstrapSuccess({ source: 'desk_sip_from' });
    return { tenantId: deskOutboundTenantId, source: 'desk_sip_from' };
  }

  if (
    String(normalized.state || '').toLowerCase() === 'parked'
    && isOutboundDirection(normalized.direction)
  ) {
