...
/**
 * True when a router has completed and persisted its route plan (not bootstrap-only snapshot).
 * @param {unknown} routeSnapshot
 */
function isRoutedRouteSnapshot(routeSnapshot) {
  return Boolean(
    routeSnapshot
    && typeof routeSnapshot === 'object'
    && /** @type {Record<string, unknown>} */ (routeSnapshot).routedAt,
  );
}
