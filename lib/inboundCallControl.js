...
/**
 * Ring-first strategy: defer PSTN answer until a desk SIP endpoint answers (200 OK).
 * Applies only to direct desk / hunt-group (sip-only) rings without media preambles.
 */
...
async function completeDeferredPstnAnswerAndBridge(
  prisma,
  session,
  ...
  await bridgeCalls(inboundCallControlId, { otherCallControlId: legCallControlId });
  logBridgeEvent('DEFERRED_PSTN_ANSWER_AND_BRIDGE', session, { legCallControlId });
  return true;
}
...
async function onOutboundLegAnswered(
  ...
  if (
    isDialAnswerEvent(eventType)
    && !isBridgeConfirmationEvent(eventType)
    && session.deferPstnAnswerUntilAgent
    && !session.pstnAnswered
  ) {
    try {
      await completeDeferredPstnAnswerAndBridge(
        prisma,
        session,
        inboundCallControlId,
        legCallControlId,
      ...
      const telnyxDetail = error.telnyx?.errors?.[0]?.detail;
      console.error(
        `   ↳ Deferred PSTN answer/bridge failed (${error.status || 'error'}): ${telnyxDetail || error.message}`,
      );
      return;
    }
...
module.exports = {
  ...
  completeDeferredPstnAnswerAndBridge,
  startConnectFlow,
