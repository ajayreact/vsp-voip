...
async function startConnectFlow(session, prisma, { skipAnnouncements = false } = {}) {
  ...
  const announcementTargets = session.ringTargets?.length
    ? session.ringTargets
    : session.preResolvedTargets || [];
  const deskFirstSkipAnnouncements = skipsPreConnectAnnouncements(announcementTargets);
  const preamble = [];

  if (
    !skipAnnouncements
    && !deskFirstSkipAnnouncements
...
function shouldDeferPstnAnswerUntilDesk({
  ...
  if (!skipsPreConnectAnnouncements(targets)) {
    if (greeting.playGreetingBeforeConnect !== false) return false;
    if (
      greeting.playCallRecordingNotice !== false
      && greeting.callRecordingEnabled !== false
    ) {
...
async function handleCallInitiated(prisma, payload, platform) {
  ...
  if (greeting?.playGreetingBeforeConnect !== false && !skipsPreConnectAnnouncements(targets)) {
