...
async function startVoicemailCapture(session) {
  ...
  // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
  console.log('TRACE startVoicemailCapture: exact function that triggers voicemail prompt/recording', {
    callControlId,
...
async function routeToVoicemailOrHangup(session, { reason, fallbackTrigger } = {}) {
  ...
  // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
  console.log('TRACE routeToVoicemailOrHangup: reason voicemail is being considered', {
  ...
  if (voicemailAllowed) {
    // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
    console.log('TRACE routeToVoicemailOrHangup: routing decision — voicemail selected', {
      callControlId: session?.callControlId || null,
      groupVmEnabled,
      tenantVmEnabled,
...
async function dialNextTarget(session, prisma) {
  ...
  // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
  console.log('TRACE dialNextTarget: SIP URI generated', {
    callControlId,
    ringIndex,
    targetType: target.type,
    sipUriGenerated: dialTo,
    dialDestinationInvoked: Boolean(dialTo),
  });

  if (!dialTo) {
    console.log('TRACE dialNextTarget: dialDestination() skipped — no dialTo resolved', {
      callControlId,
      ringIndex,
      targetType: target.type,
    });
    ...
    console.log('TRACE dialNextTarget: routing decision — voicemail (no more targets, no dialTo)', { callControlId });
    await routeToVoicemailOrHangup(session);
    return;
  }
  ...
  // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
  // Serialized to a single line (JSON.stringify) so it is not dropped by
  // `grep -E "TRACE "` filtering out multi-line console.log object dumps.
  console.log('TRACE dialDestination: request ' + JSON.stringify({
    callControlId,
    ringIndex,
    targetType: target.type,
    dialRequestPayload,
  ...
  try {
    ...
    // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
    console.log('TRACE dialDestination: Telnyx API response (success) ' + JSON.stringify({
      callControlId,
      ringIndex,
      returnedCallControlId: dialResult?.call_control_id ?? null,
      recordType: dialResult?.record_type ?? null,
      rawResult: dialResult ?? null,
    }));
    ...
    const telnyxDetail = error.telnyx?.errors?.[0]?.detail;
    const telnyxCode = error.telnyx?.errors?.[0]?.code;
    console.error(
      `   ↳ Call Control dial failed (${error.status || 'error'}): ${telnyxDetail || error.message}`,
    );

    // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
    console.log('TRACE dialDestination: exception caught ' + JSON.stringify({
      callControlId,
      ringIndex,
      targetType: target.type,
      dialToAttempted: dialTo,
      httpStatus: error.status ?? null,
      telnyxErrorCode: telnyxCode ?? null,
      telnyxErrorDetail: telnyxDetail ?? null,
      telnyxRawBody: error.telnyx ?? null,
      errorCode: error.code ?? null,
      isTimeout: error.code === 'ECONNABORTED' || /timeout/i.test(error.message || ''),
    ...
    if (session.ringIndex < (session.ringTargets?.length || 0)) {
      // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
      console.log('TRACE dialDestination: retrying next ring target after exception ' + JSON.stringify({
        callControlId,
        nextRingIndex: session.ringIndex,
        ringTargetsLength: session.ringTargets?.length || 0,
      }));
      await saveSession(callControlId, session);
      await dialNextTarget(session, prisma);
      return;
    ...
    // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
    console.log('TRACE dialDestination: routeToVoicemailOrHangup() invoked — reason: dial exception, no more ring targets ' + JSON.stringify({
      callControlId,
      ringIndex,
      targetType: target.type,
      dialToAttempted: dialTo,
    ...
    await routeToVoicemailOrHangup(session, {
      reason: `dial_exception: ${telnyxDetail || error.message || 'unknown error'}`,
      fallbackTrigger: 'dial_exception',
    });
  }
}
...
async function handleHangup(prisma, payload) {
  ...
  if (session && inboundCallControlId) {
    ...
    if ((session.stage === 'ringing' || session.stage === 'connecting') && isOutboundLeg) {
      ...
      // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
      console.log('TRACE handleHangup: exact function triggering voicemail — sequential ring targets exhausted', {
        inboundCallControlId,
        legCallControlId,
        ringIndexAfterIncrement: session.ringIndex,
        ringTargetsLength: session.ringTargets?.length || 0,
        ringTargetTypes: (session.ringTargets || []).map((t) => t.type),
        callKind: session.callKind || null,
        targetExtensionNumber: session.targetExtensionNumber || null,
      });
      await routeToVoicemailOrHangup(session, { fallbackTrigger: 'no_answer' });
