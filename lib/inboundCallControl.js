...
async function cancelRemainingOutboundLegs(session, winningLegId) {
  ...
  for (const leg of legs) {
    if (leg.callControlId && leg.callControlId !== winningLegId && leg.status === 'ringing') {
      try {
...
async function onOutboundLegAnswered(
  ...
  if (session.stage === 'bridged' && session.connectedLeg === legCallControlId) {
...
async function handleHangup(prisma, payload) {
  ...
  if (session && inboundCallControlId) {
    ...
    if (isInboundLeg && session.stage === 'ringing' && isSimultaneousStrategy(session)) {
