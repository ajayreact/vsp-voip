...
async function handleInboundCallControlEvent(prisma, body, options = {}) {
  ...
  const { isV3TelephonyWebhookIngress, isV3CallControlApplicationOutbound } = require('./telephony/PayloadNormalizer');

  if (isV3TelephonyWebhookIngress(payload)) {
    ...
    if (ingress.accepted || ingress.duplicate) {
      console.log('[CALL CONTROL] V3 app outbound bridged to ingress', {
        ...
        webhookSource: options.webhookSource || 'call-control-handler',
      });
    ...
    case 'call.initiated':
      if (String(payload.direction || '').toLowerCase() !== 'incoming') {
        ...
        if (isV3CallControlApplicationOutbound(payload)) {
          console.log('[CALL CONTROL] skipping legacy parked outbound — V3 owns this call', {
            callControlId: payload.call_control_id,
        ...
        await handleParkedWebRtcOutboundInitiated(
          prisma,
          payload,
          platform,
          { webhookSource: options.webhookSource || 'call-control-handler' },
