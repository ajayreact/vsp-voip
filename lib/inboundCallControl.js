...
function buildAgentDialClientState(session, target, targetIndex, extra = {}) {
  ...
  return encodeClientState({
    ...
    endpointType: target?.endpointType || null,
    deviceRingStrategy: target?.deviceRingStrategy || null,
    targetIndex,
    pstnCaller,
...
async function routeToVoicemailOrHangup(session, { reason, fallbackTrigger } = {}) {
  ...
  if (session.ringGroupId && !voicemailAllowed) {
    ...
    if (await hangupDeferredPstnIfUnanswered(session)) {
      return;
    }
    await speakCall(
  ...
  if (voicemailAllowed) {
    const { getPrisma } = require('../db');
    const prisma = await getPrisma();
    await ensurePstnAnsweredForMedia(session, prisma);
...
/**
 * Internal extension calls (callKind: 'internal' | 'internal_api') are a desk/app
 * originating an outbound attempt to another extension — not a real inbound PSTN
 * call — so a failed/unanswered attempt must never surface as an inbound missed
 * call. Classification reuses the existing outbound taxonomy from callLogMeta.js
 * (direction: 'outbound', callType: 'outbound_no_answer') instead of 'missed'.
 */
function isInternalCallSession(session) {
  return session?.callKind === 'internal' || session?.callKind === 'internal_api';
}

async function logInboundCall(prisma, session, status = 'completed') {
  if (!session?.callSessionId) return;

  const isInternal = isInternalCallSession(session);
  const direction = isInternal ? 'outbound' : 'inbound';

  let logStatus = status;
  let callType = status === 'completed' ? (isInternal ? 'outbound' : 'answered') : (isInternal ? 'outbound_no_answer' : 'missed');
  if (session.stage === 'bridged' || status === 'completed') {
    logStatus = 'completed';
    callType = isInternal ? 'outbound' : 'answered';
    ...
    logStatus = 'no-answer';
    callType = isInternal ? 'outbound_no_answer' : 'missed';
  }
...
async function startConnectFlow(session, prisma, { skipAnnouncements = false } = {}) {
  const { traceOutboundIfInternalSession } = require('./telephony/outboundRuntimeTrace');
  traceOutboundIfInternalSession(session, 'startConnectFlow', {
    stage: session?.stage ?? null,
    skipAnnouncements,
    preResolvedTargetCount: session?.preResolvedTargets?.length ?? 0,
  });
  const { greeting, tenant, phoneRecord, callControlId, credentialConnectionId } = session;
  const mediaPolicy = session.preConnectMediaPolicy || PRE_CONNECT_MEDIA_POLICY.NONE;
  const preamble = [];

  if (
    !skipAnnouncements
    && !session.recordingNoticePlayed
    && policyPlaysRecordingNotice(mediaPolicy)
  ...
  if (preamble.length && !session.deferPstnAnswerUntilAgent) {
    if (session.recordingNoticePlayed) {
      await startRinging(session, prisma);
      return;
    }
    session.stage = 'preamble';
    session.pendingConnect = true;
    session.recordingNoticePlayed = true;
    await saveSession(callControlId, session);
...
function sharesInboundCallSession(session, payload) {
  const sessionId = session?.callSessionId;
  const payloadSessionId = payload?.call_session_id
    ? String(payload.call_session_id).trim()
    : null;
  return Boolean(sessionId && payloadSessionId && sessionId === payloadSessionId);
}

async function isInboundAgentAnswerLeg(session, inboundCallControlId, legCallControlId, payload) {
  if (!legCallControlId || legCallControlId === inboundCallControlId) return false;
  if (isOutboundLegEvent(session, legCallControlId)) return true;
  const indexedParent = await resolveInboundIdFromLeg(legCallControlId);
  if (indexedParent === inboundCallControlId) return true;
  if (sharesInboundCallSession(session, payload)) return true;
  return false;
}

function addAgentLegIfAbsent(session, legCallControlId, { targetIndex, targetType } = {}) {
  const legs = getOutboundLegs(session);
  if (legs.some((leg) => leg.callControlId === legCallControlId)) return legs;

  const idx = typeof targetIndex === 'number' ? targetIndex : (session.ringIndex ?? 0);
  const placeholderIdx = legs.findIndex(
    (leg) => leg.targetIndex === idx && !leg.callControlId,
  );
  if (placeholderIdx >= 0) {
    legs[placeholderIdx] = {
      ...legs[placeholderIdx],
      callControlId: legCallControlId,
      status: 'ringing',
    };
  } else {
    legs.push({
      callControlId: legCallControlId,
      targetIndex: idx,
      targetType: targetType || session.ringTargets?.[idx]?.type || session.activeTarget?.type || 'sip',
      status: 'ringing',
    });
  }
  session.outboundLegs = legs;
...
async function ensureAgentLegInSession(inboundCallControlId, legCallControlId, options = {}) {
  const session = await getSession(inboundCallControlId);
  if (!session || !legCallControlId) return;
  if (isOutboundLegEvent(session, legCallControlId)) return;
  addAgentLegIfAbsent(session, legCallControlId, options);
  if (!session.outboundLegCallControlId) {
...
function resolveAgentRingFrom(session) {
  if (session.callKind === 'internal') {
    return session.from || session.callerFrom || session.to;
  }
  return session.from || session.to;
}

function resolveAgentRingDisplayName(session) {
  return session.callerDisplayName || session.from || undefined;
}

/**
 * Ring-first (Telnyx Find Me pattern): defer PSTN answer when ring strategy requires it.
 * Uses deviceRingStrategy + endpoint inventory, not target.type (app/sip routing model).
 */
function shouldDeferPstnAnswerUntilDesk({
  targets,
  ringsMobileApp = false,
  extPolicy = null,
  ...
  if (ringsMobileApp) return false;
  if (ivrWouldRun) return false;
  if (businessHoursClosed) return false;
  if (extPolicy?.action && extPolicy.action !== 'ring') return false;
  if (!Array.isArray(targets) || !targets.length) return false;
  if (!usesRingFirstPath(targets)) {
    // Pilot-tenant opt-in only (see lib/telephony/deskRingFirstPilot.js) — every other
    // tenant keeps relying solely on usesRingFirstPath/deviceRingStrategy, unchanged.
    const { isDeskOnlyRingFirstPilotTenant } = require('./telephony/deskRingFirstPilot');
    const { hasDeskCapableTarget } = require('./ringTargetPolicy');
    const allTargetsDeskOnly = targets.every((target) => hasDeskCapableTarget(target));
    if (!isDeskOnlyRingFirstPilotTenant(tenantId) || !allTargetsDeskOnly) return false;
  }
  if (preConnectMediaPolicy !== PRE_CONNECT_MEDIA_POLICY.NONE) return false;
  return true;
}

function resolveDialBridgeOnAnswer(session) {
  return !session?.deferPstnAnswerUntilAgent;
}

async function ensureInboundCallLogged(prisma, session) {
  if (!session || session.inboundCallLogged) return;
  await logInboundCallStart(prisma, session);
  session.inboundCallLogged = true;
  session.pstnAnswered = true;
  await saveSession(session.callControlId, session);
}

async function ensurePstnAnsweredForMedia(session, prisma) {
  if (!session?.deferPstnAnswerUntilAgent || session.pstnAnswered) return;
  await answerCall(
    session.callControlId,
  ...
  session.pstnAnswered = true;
  await saveSession(session.callControlId, session);
  await ensureInboundCallLogged(prisma, session);
}

async function hangupDeferredPstnIfUnanswered(session) {
  if (!session?.deferPstnAnswerUntilAgent || session.pstnAnswered) return false;
  await hangupCall(session.callControlId);
  return true;
}

/**
 * Telnyx Find Me accept path: bridge answered agent leg to parked PSTN — no answer before bridge.
 * @see docs/telnyx/call-control/guides/ivr-demo.md (gather ended, DTMF 1 → bridge only)
 */
async function bridgeParkedInboundToAgentLeg(session, inboundCallControlId, legCallControlId) {
  ...
  logBridgeEvent('RING_FIRST_BRIDGE', session, { inboundCallControlId, legCallControlId });
}
...
async function dialSingleTarget(session, target, targetIndex, ringTimeout) {
  ...
  const { traceOutboundIfInternalSession } = require('./telephony/outboundRuntimeTrace');
  traceOutboundIfInternalSession(session, 'dialDestination_request', {
    dialTo,
  ...
  const dialResult = await dialDestination(session.callControlId, {
    ...
    bridgeOnAnswer: resolveDialBridgeOnAnswer(session),
  });

  await indexAgentRingIfApplicable(session, target);

  traceOutboundIfInternalSession(session, 'dialDestination_response', {
    ok: Boolean(dialResult?.call_control_id),
...
async function startRinging(session, prisma) {
  const { traceOutboundIfInternalSession } = require('./telephony/outboundRuntimeTrace');
  traceOutboundIfInternalSession(session, 'startRinging', {
    ringTargetCount: session?.ringTargets?.length ?? session?.preResolvedTargets?.length ?? 0,
    ringStrategy: session?.ringStrategy ?? null,
  });
...
function isAgentAnswerEvent(eventType) {
  return eventType === 'call.answered';
}
...
async function onOutboundLegAnswered(
  ...
  // call.answered: agent leg picked up — ring-first bridges here; all paths wait for call.bridged.
  if (isAgentAnswerEvent(eventType) && !isBridgeConfirmationEvent(eventType)) {
    markOutboundLeg(session, legCallControlId, 'answered');
    await saveSession(inboundCallControlId, session);

    if (session.deferPstnAnswerUntilAgent && !session.pstnAnswered) {
      try {
        await bridgeParkedInboundToAgentLeg(session, inboundCallControlId, legCallControlId);
      } catch (error) {
        const telnyxDetail = error.telnyx?.errors?.[0]?.detail;
        console.error(
          `   ↳ Ring-first bridge failed (${error.status || 'error'}): ${telnyxDetail || error.message}`,
        );
        return;
      }
...
async function handleCallInitiated(prisma, payload, platform) {
  ...
  session.preConnectMediaPolicy = preConnectMediaPolicy;

  const deferPstnAnswer = shouldDeferPstnAnswerUntilDesk({
    targets,
    ringsMobileApp,
    extPolicy,
    businessHoursClosed,
  ...
  session.deferPstnAnswerUntilAgent = deferPstnAnswer;
  await saveSession(callControlId, session);

  if (deferPstnAnswer) {
    console.log('[CALL CONTROL] ring-first: deferring PSTN answer until desk SIP answers', {
      callControlId,
  ...
  const ivrOptionsResolved = normalizeIvrOptions(greeting?.ivrOptions);
  if (greeting?.ivrEnabled && ivrOptionsResolved.length && !ringsMobileApp) {
  ...
  if (
    ...
    || preConnectMediaPolicy === PRE_CONNECT_MEDIA_POLICY.GREETING_AND_RECORDING
  ) {
  ...
  const skipAnnouncements = preConnectMediaPolicy === PRE_CONNECT_MEDIA_POLICY.NONE;
  await startConnectFlow(session, prisma, { skipAnnouncements });
}
...
async function handleSpeakEnded(prisma, payload) {
  ...
  if (session.stage === 'preamble' || session.pendingConnect) {
  ...
  if (session.stage === 'greeting') {
    const skipAnnouncements = session.preConnectMediaPolicy !== PRE_CONNECT_MEDIA_POLICY.GREETING_AND_RECORDING;
    await startConnectFlow(session, prisma, { skipAnnouncements });
    return;
  }
...
async function handleDialAnswered(prisma, payload) {
  // Best-effort fallback — Telnyx Dial lifecycle documents call.answered, not call.dial.answered.
  const { session, inboundCallControlId } = await findSession(payload);
  if (!session || !inboundCallControlId) return;

  const legCallControlId = payload.call_control_id;
  const isAgentLeg = await isInboundAgentAnswerLeg(
    session,
    inboundCallControlId,
    legCallControlId,
    payload,
  );
  if (!isAgentLeg) return;

  logCallControlDiagnostics('call.dial.answered (fallback)', payload, session, inboundCallControlId);
...
async function handleCallAnswered(prisma, payload) {
  ...
  if (!isAgentLeg || isInboundLeg) return false;

  if (!isOutboundLegEvent(session, legCallControlId)) {
    await ensureAgentLegInSession(inboundCallControlId, legCallControlId);
    const refreshed = await getSession(inboundCallControlId);
    if (refreshed) session = refreshed;
  }
...
/**
 * Agent ring legs from inbound dialDestination (link_to + bridge_on_answer) must not
 * enter desk-outbound routing — only index for webhook correlation.
 */
async function handleInboundAgentDialLegInitiated(payload) {
  const legCallControlId = payload?.call_control_id
  ...
  if (direction !== 'outgoing' && direction !== 'outbound') return false;
  ...
  const rawTo = payload?.to ?? null;
  const linkTo = payload?.link_to ? String(payload.link_to).trim() : null;
  const credentialUser = parseCredentialSipUsername(rawTo);
  const isLikelyAgentDialLeg = Boolean(
    linkTo || credentialUser || isTelnyxCredentialSipDestination(rawTo),
  );
  ...
  const diag = {
    ...
    linkToSessionFound: false,
    pendingAgentRingInboundId: null,
    findSessionInboundId: null,
  };

  let inboundCallControlId = null;

  if (linkTo) {
    const linkedSession = await getSession(linkTo);
    diag.linkToSessionFound = Boolean(linkedSession);
    if (linkedSession) {
      inboundCallControlId = linkTo;
    }
  }

  if (!inboundCallControlId) {
    const indexedInboundId = await resolveInboundIdFromLeg(legCallControlId);
    diag.resolveInboundIdFromLeg = indexedInboundId;
    if (indexedInboundId) {
      const indexedSession = await getSession(indexedInboundId);
      if (indexedSession) {
        inboundCallControlId = indexedInboundId;
      }
    }
  }

  if (!inboundCallControlId && credentialUser) {
    const pending = await resolvePendingAgentRing(credentialUser);
    diag.pendingAgentRingInboundId = pending?.inboundCallControlId || null;
    if (pending?.inboundCallControlId) {
      const pendingSession = await getSession(pending.inboundCallControlId);
      if (pendingSession) {
        inboundCallControlId = pending.inboundCallControlId;
      }
    }
  ...
  if (!inboundCallControlId && isTelnyxCredentialSipDestination(rawTo)) {
    const { session, inboundCallControlId: foundId } = await findSession(payload);
    diag.findSessionInboundId = foundId || null;
    if (session && foundId && foundId !== legCallControlId) {
      inboundCallControlId = foundId;
    }
  }

  if (!inboundCallControlId) {
    if (isLikelyAgentDialLeg) {
      console.log('[CALL CONTROL] inbound agent dial leg guard: no parent session (falling through to desk outbound handler)', diag);
    }
    return false;
  }

  await indexOutboundLeg(inboundCallControlId, legCallControlId);
  await ensureAgentLegInSession(inboundCallControlId, legCallControlId);
  console.log('[CALL CONTROL] inbound agent dial leg indexed (skip desk outbound routing)', {
    inboundCallControlId,
    legCallControlId,
    linkTo: linkTo || null,
    credentialUser: credentialUser || null,
...
/**
 * Voice webhook may emit call.initiated (incoming) with to = Telnyx credential username
 * when an inbound agent ring reaches a desk/app SIP endpoint. That leg belongs to an
 * existing inbound Call Control session — not a new PSTN DID call.
 */
async function handleInboundAgentCredentialRingInitiated(payload) {
  const direction = String(payload.direction || '').toLowerCase();
  ...
  const rawCredentialUser = parseCredentialSipUsername(payload?.to)
    || (looksLikeTelnyxCredentialUsername(payload?.to) ? String(payload.to).trim() : null);
  const credentialUser = rawCredentialUser ? rawCredentialUser.toLowerCase() : null;
  ...
  await indexOutboundLeg(inboundCallControlId, legCallControlId);
  await ensureAgentLegInSession(inboundCallControlId, legCallControlId);
  console.log('[CALL CONTROL] inbound agent credential ring indexed (skip PSTN inbound routing)', {
    inboundCallControlId,
    legCallControlId,
    credentialUser,
    linkTo: linkTo || null,
  ...
  switch (eventType) {
    case 'call.initiated':
      if (String(payload.direction || '').toLowerCase() !== 'incoming') {
        ...
        if (await handleInboundAgentDialLegInitiated(payload)) break;
        ...
        if (await handleInboundAgentCredentialRingInitiated(payload)) break;
...
module.exports = {
  ...
  handleInboundAgentDialLegInitiated,
  handleInboundAgentCredentialRingInitiated,
  handleCallAnswered,
  shouldDeferPstnAnswerUntilDesk,
  usesRingFirstPath,
  usesOptionARingPath,
  resolveDialBridgeOnAnswer,
