...
async function handleHangup(prisma, payload) {
  ...
  if (session && inboundCallControlId) {
    ...
    // Caller abandoned while targets are still ringing — cancel every outbound ring
    // leg (desk/app) before tearing down the session. Include stage=connect because
    // startConnectFlow sets that before dialNextTarget promotes to ringing; a fast
    // PSTN hangup in that window previously left the desk phone ringing.
    const callerAbandonedDuringRing = isInboundLeg
      && !activeWinner
      && ['connect', 'ringing'].includes(session.stage);
