...
async function handleHangup(prisma, payload) {
  ...
  if (session && inboundCallControlId) {
    ...
    // leg (desk/app) before tearing down the session. Previously gated to
    // simultaneous strategy only, so the common single-desk sequential path left
    // the desk phone ringing after the PSTN caller hung up.
    if (isInboundLeg && session.stage === 'ringing' && !activeWinner) {
