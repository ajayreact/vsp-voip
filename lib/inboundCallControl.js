...
/** Temporary Bug #7 QA — structured Call Control timeline (server-side). */
const BUG7_CC_EVENTS = new Set([
  'call.initiated',
  'call.ringing',
  'call.answered',
  'call.bridged',
...
function logBug7CallControl(eventType, payload, extra = {}) {
  if (!BUG7_CC_EVENTS.has(eventType)) return;
  console.log('[vsp-telephony-cc]', JSON.stringify({
...
function resolveAgentSipUsernameForSession(session, winnerLegId) {
  const fromActive = extractSipUsernameFromTarget(session?.activeTarget);
  if (fromActive) return fromActive;

  const leg = findOutboundLeg(session, winnerLegId);
  if (leg?.targetIndex != null) {
    const fromLegTarget = extractSipUsernameFromTarget(session.ringTargets?.[leg.targetIndex]);
    if (fromLegTarget) return fromLegTarget;
  }

  for (const target of session?.ringTargets || []) {
    const sip = extractSipUsernameFromTarget(target);
    if (sip) return sip;
  }
  return null;
}
...
/**
 * Pilot ring-first (Issue 1) leaves the PSTN leg unanswered so Telnyx's native
 * bridge_on_answer/link_to dial can bridge it directly once the desk phone answers.
 * Any path that needs to play a prompt (voicemail, no-answer, forwarding failure)
 * must answer the still-parked leg first, since speak/gather require an answered call.
 * No-op for every call that was answered eagerly (i.e. every non-pilot call, unchanged).
 */
async function ensurePstnAnsweredForMedia(session) {
  if (!session || session.pstnAnswered || !session.deferPstnAnswerUntilAgent) return;
  session.pstnAnswered = true;
  session.deferPstnAnswerUntilAgent = false;
  await saveSession(session.callControlId, session);
  await answerCall(
    session.callControlId,
    encodeClientState({ tenantId: session.tenantId, direction: 'inbound' }),
  );
...
/**
 * Ring-first accept path: bridge the answered desk leg to the still-parked
 * (never-answered) PSTN leg. dialDestination() was called with bridgeOnAnswer:
 * false for this leg (see dialSingleTarget/dialNextTarget), so Telnyx will not
 * auto-bridge — this explicit bridge is required. Mirrors the documented
 * pattern in docs/vsp/pbx/21-event-sequence.md ("Telnyx Find Me pattern").
 */
async function bridgeParkedInboundToAgentLeg(session, inboundCallControlId, legCallControlId) {
  const { isCallerAbandoned } = require('./telephony/callerAbandonLifecycle');
  const liveSession = await getSession(inboundCallControlId);
  if (!liveSession || isCallerAbandoned(liveSession)) {
    console.log('   ↳ Ring-first bridge blocked — PSTN caller already abandoned or session closed', {
      inboundCallControlId,
      legCallControlId,
      sessionExists: Boolean(liveSession),
      callerAbandoned: isCallerAbandoned(liveSession || session),
    });
    try {
      await hangupCall(legCallControlId);
    } catch (error) {
      console.warn(`   ↳ Could not hang up late-answered desk leg ${legCallControlId}: ${error.message}`);
    }
    return false;
  }
  await bridgeCalls(legCallControlId, { otherCallControlId: inboundCallControlId });
  liveSession.pstnAnswered = true;
  await saveSession(inboundCallControlId, liveSession);
  console.log('   ↳ Call Control: ring-first bridge (parked PSTN leg → answered desk leg)', {
    inboundCallControlId,
    legCallControlId,
  });
  return true;
...
async function startVoicemailCapture(session) {
  ...
  await ensurePstnAnsweredForMedia(session);
...
async function cancelAllRingingOutboundLegs(session) {
  ...
  } = require('./telephony/callerAbandonLifecycle');
  const legs = getOutboundLegs(session);
  for (const leg of legs) {
    if (leg.callControlId && leg.status === 'ringing') {
      markLegCancelRequested(session, leg.callControlId);
      try {
        await hangupCall(leg.callControlId);
      } catch (error) {
        console.warn(`   ↳ Could not cancel outbound leg ${leg.callControlId}: ${error.message}`);
...
/** Caller abandon — hang up every non-answered outbound leg, including legs already marked cancelled. */
async function hangupOutboundLegsForCallerAbandon(session) {
  const { markLegCancelRequested } = require('./telephony/callerAbandonLifecycle');
  const legIds = new Set();
  for (const leg of getOutboundLegs(session)) {
    if (leg.callControlId && leg.status !== 'answered') {
      legIds.add(leg.callControlId);
    }
  }
  if (session.outboundLegCallControlId && session.connectedLeg !== session.outboundLegCallControlId) {
    legIds.add(session.outboundLegCallControlId);
  }

  for (const legCallControlId of legIds) {
    markLegCancelRequested(session, legCallControlId);
    try {
      await hangupCall(legCallControlId);
    } catch (error) {
      console.warn(`   ↳ Could not hang up abandoned outbound leg ${legCallControlId}: ${error.message}`);
    }
    markOutboundLeg(session, legCallControlId, 'cancelled');
  }

  session.dialInFlight = null;
  if (session.callControlId) {
    await saveSession(session.callControlId, session);
  }
}

async function abandonInboundCaller(prisma, inboundCallControlId, session, payload = null) {
  session.callerAbandoned = true;
  session.callerAbandonedAt = Date.now();
  await saveSession(inboundCallControlId, session);
  console.log('   ↳ Call Control: PSTN caller abandoned — cancelling desk ring', {
    inboundCallControlId,
    stage: session.stage,
    ringIndex: session.ringIndex ?? 0,
    targetCount: session.ringTargets?.length ?? 0,
    hangup_cause: payload?.hangup_cause ?? null,
    hangup_source: payload?.hangup_source ?? null,
  });
  await hangupOutboundLegsForCallerAbandon(session);
...
/** Read-only lookup for ringing UI — does not mutate session stage. */
async function getPendingInboundCallerForAgent(sipUsername) {
  const normalizedSip = String(sipUsername || '').trim().toLowerCase();
  const pending = await resolvePendingAgentRing(normalizedSip);
  ...
  return {
    ...
    pstnCallerName: session?.callerDisplayName || null,
...
async function markAgentWebRtcAccepted(sipUsername) {
  ...
  const pending = await resolvePendingAgentRing(normalizedSip);
  if (!pending?.inboundCallControlId) {
    console.warn('[call-accepted] markAgentWebRtcAccepted: no pending ring', {
      sipUsername: normalizedSip,
    });
    return { ok: false, reason: 'no_pending_ring' };
  }
  ...
  const session = await getSession(pending.inboundCallControlId);
  if (!session) {
    console.warn('[call-accepted] markAgentWebRtcAccepted: session missing', {
      sipUsername: normalizedSip,
      inboundCallControlId: pending.inboundCallControlId,
    });
...
/** Caller ID presented on agent/desk ring legs. Inbound PSTN must show the external caller, not the DID. */
function resolveAgentRingFrom(session) {
  if (session.callKind === 'internal') {
    if (session.agentRingFrom) return session.agentRingFrom;
    const raw = session.from || session.callerFrom || session.to;
    // ext:NNN is an internal label only — Telnyx dial rejects it and the B-leg
    // shows +10000000000 then hangs up before the desk phone rings.
    if (String(raw).startsWith('ext:')) {
      return session.to;
    }
    return raw;
  }
...
// Telnyx `from_display_name` only allows letters, numbers, spaces, and
// -_~!.+ (see docs/telnyx/call-control/api-reference/call-commands/dial.md).
// Internal caller labels like "ext:100" contain a colon, which Telnyx's API
// rejects with HTTP 422, causing dialDestination() to throw and the call to
// fall through to voicemail before the destination is ever dialed.
const TELNYX_DISPLAY_NAME_DISALLOWED_CHARS = /[^A-Za-z0-9 \-_~!.+]/g;

function sanitizeDisplayName(value) {
  if (!value) return undefined;
  const sanitized = String(value)
    .replace(TELNYX_DISPLAY_NAME_DISALLOWED_CHARS, '')
    .trim()
    .slice(0, 128);
  return sanitized || undefined;
}

function resolveAgentRingDisplayName(session) {
  return sanitizeDisplayName(session.callerDisplayName || session.from);
}
...
async function dialSingleTarget(session, target, targetIndex, ringTimeout) {
  ...
  const dialFrom = resolveAgentRingFrom(session);

  const dialResult = await dialDestination(session.callControlId, {
    to: dialTo,
    from: dialFrom,
    fromDisplayName: resolveAgentRingDisplayName(session),
    ...
    // Ring-first: PSTN leg is intentionally left parked, so it cannot be
    // auto-bridged on answer — bridged manually once the desk leg answers
    // (see bridgeParkedInboundToAgentLeg in onOutboundLegAnswered).
    bridgeOnAnswer: !session.deferPstnAnswerUntilAgent,
  });

  await indexAgentRingIfApplicable(session, target);

  // Desk -> extension re-dial loop fix (same root cause as the Desk -> PSTN
  // guard in PstnCallService.js): dialDestination() creates a brand-new
  // Telnyx leg on the same Call Control Application connection that genuine
  // desk-originated calls arrive on. Telnyx fires its own call.initiated
  // webhook for that new leg, which handleParkedWebRtcOutboundInitiated()
  // cannot distinguish from a fresh desk call unless it is marked here,
  // immediately after dialDestination() returns. Applies to every ring
  // target (inbound PSTN -> app/desk, and internal extension -> extension),
  // since none of those dialed legs are ever genuine new desk-originated
  // calls. See lib/telephony/deskOutboundLegGuard.js for the full story.
  if (dialResult?.call_control_id) {
    const { markDeskOutboundChildLeg } = require('./telephony/deskOutboundLegGuard');
    await markDeskOutboundChildLeg(dialResult.call_control_id);
  }
...
async function markSessionConnecting(session, inboundCallControlId, legCallControlId, eventType) {
  ...
  // CALL_BRIDGED is logged only after winner claim in markSessionBridged().
  if (eventType !== 'call.bridged') {
    logBridgeEvent('CALL_DIAL_ANSWERED', session, { legCallControlId, eventType });
  }
}

async function markSessionBridged(session, inboundCallControlId, legCallControlId, eventType, extra = {}) {
  session.stage = 'bridged';
  session.activeBridge = true;
  session.connectedAt = session.connectedAt || new Date().toISOString();
  ...
  await saveSession(inboundCallControlId, session);
  logBridgeEvent('CALL_BRIDGED', session, { legCallControlId, eventType, ...extra });

  const agentSipUsername = resolveAgentSipUsernameForSession(session, legCallControlId);
  if (agentSipUsername) {
    await indexActiveAgentCall(agentSipUsername, {
      inboundCallControlId,
      agentLegId: legCallControlId,
      callSessionId: session.callSessionId || null,
      tenantId: session.tenantId || null,
      callerLegId: inboundCallControlId,
    });
  }
}
...
async function onOutboundLegAnswered(
  ...
  if (!freshSession || isCallerAbandoned(freshSession)) {
    console.log('   ↳ Ignoring late desk answer — PSTN caller abandoned or session closed', {
      inboundCallControlId,
      legCallControlId,
      eventType: eventType || null,
      sessionExists: Boolean(freshSession),
      callerAbandoned: isCallerAbandoned(freshSession || session),
    });
    try {
      await hangupCall(legCallControlId);
    } catch (error) {
  ...
  const leg = findOutboundLeg(session, legCallControlId);
  if (isOutboundLegTerminalForAnswer(leg)) {
    console.log(`   ↳ Ignoring answer on leg ${legCallControlId} (status=${leg?.status || 'unknown'})`);
    try {
      await hangupCall(legCallControlId);
    } catch (error) {
      console.warn(`   ↳ Could not hang up cancelled leg ${legCallControlId}: ${error.message}`);
    }
    return;
  }
  ...
  if (!postConnectSession || isCallerAbandoned(postConnectSession)) {
    console.log('   ↳ Ignoring desk answer after connect mark — caller abandoned during race', {
      inboundCallControlId,
      legCallControlId,
    });
    try {
      ...
      console.warn(`   ↳ Could not hang up desk leg ${legCallControlId}: ${error.message}`);
    }
    return;
  }
  session = postConnectSession;
  ...
  if (isDialAnswerEvent(eventType) && !isBridgeConfirmationEvent(eventType)) {
    ...
    // Ring-first: the PSTN leg was dialed with bridgeOnAnswer:false and is still
    // parked, so Telnyx will not auto-bridge it. Bridge it explicitly now that the
    // desk leg has answered; the resulting call.bridged webhook then drives the
    // normal winner-claim path below exactly like the non-deferred bridge_on_answer
    // flow does.
    if (session.deferPstnAnswerUntilAgent && !session.pstnAnswered) {
      const bridged = await bridgeParkedInboundToAgentLeg(session, inboundCallControlId, legCallControlId);
      if (!bridged) return;
      const refreshed = await getSession(inboundCallControlId);
  ...
  if (!claim.claimed) {
    if (claim.isDuplicateWinnerEvent) {
      const winnerLeg = claim.legCallControlId || legCallControlId;
      console.log(
        `   ↳ Duplicate ${eventType || 'answer'} for winner ${winnerLeg}`,
      );
      if (session.stage !== 'bridged') {
        await markSessionBridged(
          session,
          inboundCallControlId,
          winnerLeg,
          eventType,
      ...
      await applyAnswerSideEffectsOnce(prisma, session, inboundCallControlId, winnerLeg);
      return;
    }
    if (claim.lostRace) {
      const winnerLeg = claim.legCallControlId || await getClaimedWinner(inboundCallControlId);
      if (isBridgeConfirmationEvent(eventType)) {
        console.log(
          `   ↳ Ignoring call.bridged on paired leg ${legCallControlId} — bridge winner ${winnerLeg}`,
        );
        if (winnerLeg && session.stage !== 'bridged') {
          await markSessionBridged(
            session,
            inboundCallControlId,
            winnerLeg,
      ...
      console.log(
        `   ↳ Ignoring answer on leg ${legCallControlId} — winner is ${winnerLeg}`,
      );
      try {
        await hangupCall(legCallControlId);
      } catch (error) {
  ...
  await markSessionBridged(session, inboundCallControlId, legCallControlId, eventType);
...
async function dialNextTarget(session, prisma) {
  ...
  session.dialInFlight = { ringIndex, startedAt: Date.now() };
  upsertOutboundLeg(session, {
    callControlId: null,
  ...
  try {
    const dialResult = await dialDestination(callControlId, {
      to: dialTo,
      from: resolveAgentRingFrom(session),
      fromDisplayName: resolveAgentRingDisplayName(session),
    ...
    const liveSession = await getSession(callControlId);
    if (!liveSession || liveSession.callerAbandoned) {
      if (outboundLegId) {
        const { markDeskOutboundChildLeg } = require('./telephony/deskOutboundLegGuard');
        await markDeskOutboundChildLeg(outboundLegId);
        try {
          await hangupCall(outboundLegId);
        } catch (hangupError) {
          console.warn(`   ↳ Could not hang up desk leg after caller abandon ${outboundLegId}: ${hangupError.message}`);
        }
        console.log('   ↳ Discarded outbound leg — PSTN caller abandoned during dial', {
          callControlId,
          outboundLegId,
          callerAbandoned: Boolean(liveSession?.callerAbandoned),
        });
      }
      return;
    }
    ...
    if (outboundLegId) {
      // Desk -> extension re-dial loop fix — see matching comment in
      // dialSingleTarget() and lib/telephony/deskOutboundLegGuard.js. This
      // sequential-strategy dial path is the default for a single ring
      // target (e.g. one extension with one desk/app credential), so it
      // needs the same guard as the simultaneous-strategy path above.
      const { markDeskOutboundChildLeg } = require('./telephony/deskOutboundLegGuard');
      await markDeskOutboundChildLeg(outboundLegId);

      liveSession.outboundLegCallControlId = outboundLegId;
      upsertOutboundLeg(liveSession, {
        callControlId: outboundLegId,
        targetIndex: ringIndex,
        targetType: target.type,
        status: 'ringing',
      });
      liveSession.dialInFlight = null;
      await indexOutboundLeg(callControlId, outboundLegId);
      await saveSession(callControlId, liveSession);
...
async function handleCallInitiated(prisma, payload, platform) {
  ...
  // Desk -> extension/DID ring dials create extra Telnyx legs in the same
  // call_session_id. The outbound child is guarded in
  // handleParkedWebRtcOutboundInitiated(), but Telnyx also emits incoming
  // call.initiated events (often with a bogus From like +10000000000) for the
  // SIP ring leg. Those must not enter the PSTN inbound handler or they
  // re-dial, hang up, and tear down the real internal call.
  const { isDeskOutboundChildLeg } = require('./telephony/deskOutboundLegGuard');
  if (await isDeskOutboundChildLeg(callControlId)) {
    console.log('   ↳ Call Control inbound skipped: self-dialed child leg', { callControlId });
    return;
  }

  const linked = await findSession(payload);
  if (linked?.session?.callKind === 'internal' && linked.inboundCallControlId !== callControlId) {
    console.log('   ↳ Call Control inbound skipped: leg belongs to active internal extension call', {
      callControlId,
      inboundSessionId: linked.inboundCallControlId,
      callSessionId: payload.call_session_id || null,
      stage: linked.session.stage,
    });
    return;
  }
  ...
  // Internal extension dialing is handled on outbound credential-connection legs only
  // (handleParkedWebRtcOutboundInitiated). Inbound PSTN must resolve tenant from DID first.
  ...
  if (!context?.tenant) {
    ...
    await writeSystemAuditLog(prisma, {
      action: 'inbound.unassigned_did',
      entityType: 'PhoneNumber',
      entityId: context?.to || payload.to || null,
      details: {
        from: context?.from || payload.from || null,
        to: context?.to || payload.to || null,
        callControlId,
        reason: 'No tenant assigned to dialed number',
      },
    });
  ...
  // Hoisted (read-only, no side effects) so the pilot ring-first gate below can be
  // evaluated before deciding whether to answer the PSTN leg. Reused further down
  ...
  const businessHoursClosed = Boolean(
    greeting?.businessHoursEnabled
    && !isWithinBusinessHours(greeting.businessHours, tenant.timezone || 'America/New_York'),
  );
  ...
  const { shouldDeferPstnAnswerForPilot } = require('./telephony/deskRingFirstPilot');
  const deferPstnAnswer = await shouldDeferPstnAnswerForPilot({
    prisma,
    tenantId: tenant.id,
    targets,
    businessHoursClosed,
  ...
  if (deferPstnAnswer) {
    session.deferPstnAnswerUntilAgent = true;
    session.pstnAnswered = false;
    await saveSession(callControlId, session);
    console.log('   ↳ Call Control: pilot ring-first — deferring PSTN answer until desk phone answers', {
      callControlId,
      tenantId: tenant.id,
    });
  } else {
  ...
  if (greeting?.businessHoursEnabled) {
    const open = !businessHoursClosed;
    if (!open) {
...
async function handleDialEnded(prisma, payload) {
  ...
  if (shouldBlockRingProgression(session) || !(await getSession(inboundCallControlId))) {
    console.log('   ↳ Ignoring call.dial.ended — caller abandoned or session closed', {
      inboundCallControlId,
      legCallControlId,
      dialStatus,
    });
    if (legCallControlId) {
      try {
        ...
        console.warn(`   ↳ Could not hang up leg after abandoned dial.ended ${legCallControlId}: ${error.message}`);
      }
    }
    return;
  }
  ...
  // Ring-first: Telnyx may emit dial.ended with empty/cancel status when the PSTN
  // caller hangs up — that is not callee no-answer and must not advance the ring list.
  if (shouldDeferOutboundHangupAdvance(session) && !isDialProgressionStatus(dialStatus)) {
    console.log('   ↳ Ignoring call.dial.ended for ring-first — not a no-answer progression', {
      inboundCallControlId,
      legCallControlId,
      dialStatus,
    });
...
async function handleHangup(prisma, payload) {
  ...
  if (session && inboundCallControlId) {
    ...
    logCallControlDiagnostics('call.hangup', payload, session, inboundCallControlId, {
      isInboundLeg,
      isOutboundLeg,
      ...hangupOriginDiagnostics(payload, session, legCallControlId),
    });
    ...
    if (
      session.stage === 'connecting'
      && shouldIgnoreLegTerminationDuringBridgeGrace(session)
    ) {
      logBridgeEvent('CALL_HANGUP', session, {
        legCallControlId,
        ignored: true,
        isInboundLeg,
        isOutboundLeg,
    ...
    // PSTN caller disconnected before bridge — persist flag, cancel desk legs, delete session.
    const abandonSession = (await getSession(inboundCallControlId)) || session;
    const callerAbandonedDuringRing = isInboundLeg
      && !activeWinner
      && isRingInProgressStage(abandonSession.stage);
    if (callerAbandonedDuringRing) {
      await abandonInboundCaller(prisma, inboundCallControlId, abandonSession, payload);
      return;
    }

    if (shouldBlockRingProgression(session)) {
      await hangupOutboundLegsForCallerAbandon(session);
      if (await getSession(inboundCallControlId)) {
        await cleanupInboundSession(prisma, inboundCallControlId, session, 'no-answer');
      }
      return;
    }

    if (isRingInProgressStage(session.stage) && isOutboundLeg) {
      if (shouldIgnoreLegTerminationDuringBridgeGrace(session)) {
      ...
      // Ring-first: outbound hangup while PSTN is still parked is Telnyx tearing down the
      // dial leg (not callee no-answer). Do not advance to the next target or voicemail.
      if (shouldDeferOutboundHangupAdvance(session)) {
        const { markLegCancelRequested } = require('./telephony/callerAbandonLifecycle');
        console.log('   ↳ Ignoring outbound hangup for ring-first — awaiting inbound abandon or no-answer', {
          inboundCallControlId,
          legCallControlId,
          ...hangupOriginDiagnostics(payload, session, legCallControlId),
        });
        ...
        try {
          ...
          console.warn(`   ↳ Could not hang up ring-first outbound leg ${legCallControlId}: ${error.message}`);
        }
        markOutboundLeg(session, legCallControlId, 'cancelled');
        await saveSession(inboundCallControlId, session);
        return;
    ...
    if (isRingInProgressStage(session.stage) && !isInboundLeg && !isOutboundLeg) {
      console.log('   ↳ Ignoring call.hangup for unknown leg during ring — session kept for abandon/dial', {
        inboundCallControlId,
        legCallControlId,
        stage: session.stage,
        dialInFlight: Boolean(session.dialInFlight),
      });
      return;
    }
...
async function handleInboundCallControlEvent(prisma, body, options = {}) {
  pruneStaleSessions();
  ...
  const { handleTransferCallControlEvent } = require('./callTransferControl');
  const { isV3CallControlApplicationOutbound } = require('./telephony/PayloadNormalizer');

  traceDeskDesk('Webhook Received', {
    ...
    direction: payload.direction ?? null,
    state: payload.state ?? null,
    source: options.webhookSource || 'call-control-handler',
  });

  if (isV3CallControlApplicationOutbound(payload)) {
    const { evaluateV3DeskOutboundBridge } = require('./telephony/v3BridgeGuard');
    const bridgeDecision = await evaluateV3DeskOutboundBridge(prisma, payload, platform);
    if (bridgeDecision.shouldBridge) {
      const { handleV3WebhookIngress } = require('./telephony-v3/WebhookGateway/gateway');
      const ingress = await handleV3WebhookIngress(body, {
        source: 'call-control-v3-app-bridge',
      });
      if (ingress.accepted || ingress.duplicate) {
        console.log('[CALL CONTROL] V3 app outbound bridged to ingress', {
          eventType,
          callControlId: payload.call_control_id,
          tenantId: bridgeDecision.tenantId,
          duplicate: Boolean(ingress.duplicate),
          ingressId: ingress.ingressId || null,
        });
        return { eventType, callControlId: payload.call_control_id, v3Ingress: true };
      }
      console.warn('[CALL CONTROL] V3 app outbound ingress not accepted', {
        eventType,
        callControlId: payload.call_control_id,
        reason: ingress.reason || 'unknown',
      });
    } else {
      console.log('[CALL CONTROL] V3 app outbound bridge skipped — using legacy routing', {
        eventType,
        callControlId: payload.call_control_id,
        tenantId: bridgeDecision.tenantId,
  ...
  if (eventType === 'call.initiated') {
    const { logDeskOriginatedWebhookIngress } = require('./telephony/deskOutboundTrace');
    logDeskOriginatedWebhookIngress(payload, options.webhookSource || 'call-control-handler');
  }

  console.log('📲 Call Control event:', eventType, payload.from, '→', payload.to);
  logBug7CallControl(eventType, payload);

  switch (eventType) {
    case 'call.initiated':
      if (String(payload.direction || '').toLowerCase() !== 'incoming') {
        if (await handleTransferCallControlEvent(prisma, eventType, payload)) break;
        const { handleParkedWebRtcOutboundInitiated } = require('./internalExtensionDial');
        await handleParkedWebRtcOutboundInitiated(prisma, payload, platform);
      } else {
    ...
    case 'call.hangup':
      ...
      if (await handleTransferCallControlEvent(prisma, eventType, payload)) {
        try {
          await recordCallQualityFromTelnyxEvent(prisma, {
            data: { event_type: 'call.hangup', payload },
          });
...
module.exports = {
  ...
  getPendingInboundCallerForAgent,
  onOutboundLegAnswered,
  bridgeParkedInboundToAgentLeg,
  ensurePstnAnsweredForMedia,
  sanitizeDisplayName,
  resolveAgentRingDisplayName,
  resolveAgentRingFrom,
  // Exposed for regression coverage of the desk -> extension re-dial loop fix
  // (see lib/telephony/deskOutboundLegGuard.js) — not part of the public
  // webhook-handling API, but the smallest way to unit test the guard call
  // without duplicating the full inbound-call-control session machinery.
  dialSingleTarget,
  dialNextTarget,
  // Exposed for regression: PSTN caller hangup must cancel sequential desk ring
  handleHangup,
  handleDialEnded,
  handleCallAnswered,
  handleCallInitiated,
