...
async function resetUserTelephonyCredential({ prisma, userId, tenantId, connectionId }) {
  if (!TELNYX_API_KEY) {
    throw Object.assign(new Error('TELNYX_API_KEY is not configured'), { status: 500 });
  }
  if (!connectionId) {
    throw Object.assign(new Error('WebRTC credential connection is not configured'), { status: 503 });
  }
  ...
  if (user.telnyxCredentialId) {
    try {
      await deleteTelephonyCredential(user.telnyxCredentialId);
    } catch (error) {
      console.warn(`Could not delete employee Telnyx credential ${user.telnyxCredentialId}: ${error.message}`);
    }
  }
