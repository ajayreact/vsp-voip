...
async function resolveExtensionRingTargets(prisma, extension, credentialConnectionId, phoneRecord = null) {
  ...
  console.log('TRACE ring-targets: destination extension found', {
    extensionId: extension.id,
    extensionNumber: extension.extensionNumber,
    extensionUserId: extension.userId ?? null,
    effectiveUserId,
    deskDevice_telnyxSipUsername: extension.telnyxSipUsername ?? null,
    deskDevice_sipEnabled: extension.sipEnabled,
  });
  ...
  if (effectiveUserId) {
    ...
    user = await ensureAppUserDialReady(prisma, user, credentialConnectionId);
    // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
    console.log('TRACE ring-targets: destination user resolved', {
      userId: user?.id ?? null,
      userTelnyxSipUsername: user?.telnyxSipUsername ?? null,
      willAddAppTarget: Boolean(user?.telnyxSipUsername),
    });
  ...
  // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
  console.log('TRACE ring-targets: routing decision', {
    extensionNumber: extension.extensionNumber,
    appTargetCount: appTargets.length,
    sipTargetCount: sipTargets.length,
    deskDeviceSipTargetSkipped: appTargets.length > 0 && Boolean(extension.telnyxSipUsername),
    reason: appTargets.length
      ? 'app_target_present_desk_sip_excluded'
      : (sipTargets.length ? 'desk_sip_target_used' : 'no_targets_available'),
    sipUriGenerated: targets.map((t) => formatTargetDialTo(t)),
  });
