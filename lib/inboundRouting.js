...
const {
  ...
  deskEndpointMeta,
} = require('./ringTargetEndpoints');
const { resolveDeviceRingStrategy } = require('./ringTargetPolicy');
...
async function resolveAppTargetForUser(
  ...
  { extensionId = null, label = null, extension = null } = {},
) {
  ...
  if (!resolvedExtension && extensionId) {
    resolvedExtension = await prisma.extension.findFirst({
      where: { id: extensionId, tenantId, status: 'ACTIVE' },
    });
  }
  if (!resolvedExtension && userId) {
    resolvedExtension = await prisma.extension.findFirst({
      where: { tenantId, userId, status: 'ACTIVE' },
    });
  }

  const endpointMeta = resolvedExtension
    ? await resolveTargetEndpointMeta(prisma, { extension: resolvedExtension, user, routingType: 'app' })
    : await resolveTargetEndpointMeta(prisma, { extension: { id: null, userId, sipEnabled: true }, user, routingType: 'app' });

  return {
    ...annotateRingTarget({
      type: 'app',
      user,
      extensionId: extensionId || resolvedExtension?.id || null,
      label: label || user.name,
    }, endpointMeta),
    deviceRingStrategy: resolveDeviceRingStrategy(resolvedExtension),
  };
}
...
async function resolveExtensionRingTargets(prisma, extension, credentialConnectionId, phoneRecord = null) {
  ...
  if (effectiveUserId) {
    ...
    if (user?.telnyxSipUsername) {
      const endpointMeta = await resolveTargetEndpointMeta(prisma, {
        extension,
        user,
        routingType: 'app',
      });
      appTargets.push({
        ...
        deviceRingStrategy: resolveDeviceRingStrategy(extension),
      });
    }
  }

  // Phase 2.4a: assigned employees ring via User.telnyxSipUsername (app target above).
  // endpointType (desk/mobile/webrtc) is classified separately in ringTargetEndpoints.js.
  ...
  if (
    ...
    sipTargets.push({
      ...annotateRingTarget({
        type: 'sip',
        sipUsername: extension.telnyxSipUsername,
        extensionId: extension.id,
  ...
  const targets = [...appTargets, ...sipTargets];
  ...
  traceDeskDesk('resolveExtensionRingTargets()', {
    ...
    appTargets: appTargets.map((t) => ({
      type: t.type,
      endpointType: t.endpointType ?? null,
...
async function resolveGreetingRingTargets(prisma, tenantId, greeting, credentialConnectionId, ringTimeout) {
  ...
  for (const member of members) {
    if (member.type === 'app' && member.userId) {
      ...
      if (user?.telnyxSipUsername) {
        const memberExtension = await prisma.extension.findFirst({
          where: { tenantId, userId: member.userId, status: 'ACTIVE' },
        });
        const endpointMeta = memberExtension
          ? await resolveTargetEndpointMeta(prisma, { extension: memberExtension, user, routingType: 'app' })
          : await resolveTargetEndpointMeta(prisma, { extension: { id: null, userId: member.userId, sipEnabled: true }, user, routingType: 'app' });
        targets.push({
          ...annotateRingTarget({
            type: 'app',
            user,
            extensionId: memberExtension?.id || null,
            label: member.label || user.name,
          }, endpointMeta),
          deviceRingStrategy: resolveDeviceRingStrategy(memberExtension),
        });
      }
...
module.exports = {
  ...
  hasAppRingTargets,
  hasSipRingTargets,
  // Re-export endpoint helpers for inbound call control and tests.
  hasMobileOrWebRtcRingTargets: require('./ringTargetEndpoints').hasMobileOrWebRtcRingTargets,
  isDeskOnlyRingTargets: require('./ringTargetEndpoints').isDeskOnlyRingTargets,
  usesOptionARingPath: require('./ringTargetPolicy').usesOptionARingPath,
  usesRingFirstPath: require('./ringTargetPolicy').usesRingFirstPath,
  skipsPreConnectAnnouncements: require('./ringTargetPolicy').skipsPreConnectAnnouncements,
  resolveDeviceRingStrategy: require('./ringTargetPolicy').resolveDeviceRingStrategy,
  mergeRingTargets,
