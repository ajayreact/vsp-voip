...
const {
  ...
  suggestNextExtensionNumber: pickNextExtensionNumber,
} = require('./extensionNumber');
...
function deriveDeviceRows(extension, now = Date.now()) {
  ...
  if (extension.sipEnabled && user?.telnyxSipUsername) {
    const online = Boolean(user.sipRegistered);
    rows.push({
      deviceType: 'SIP',
      deviceName: `SIP (${user.telnyxSipUsername})`,
      platform: 'sip',
      userDeviceId: null,
      status: online ? 'ONLINE' : 'OFFLINE',
      lastRegistrationAt: user.sipRegistrationCheckedAt || null,
    });
  } else if (extension.sipEnabled && extension.telnyxSipUsername) {
...
async function serializeExtension(extension, devices, { forwardingDetail = null, phoneNumber = null } = {}) {
  ...
  return {
    ...
    deviceRingStrategy: extension.deviceRingStrategy || EXTENSION_DEVICE_RING_STRATEGY.SIMULTANEOUS,
...
async function suggestNextExtensionNumber(prisma, tenantId) {
  ...
  return pickNextExtensionNumber(existing.map((row) => row.extensionNumber));
}
...
async function createExtension(prisma, tenantId, body, actor = {}) {
  ...
  try {
    ...
    console.warn(`Extension ${extensionNumber}: employee Telnyx credential not created on insert: ${error.message}`);
  }
  ...
  await ensureExtensionTelephonyProvisioned(prisma, full, { stage: 'create-extension' });
...
async function updateExtension(prisma, tenantId, extensionId, body) {
  ...
  if (body.deviceRingStrategy !== undefined) {
    const strategy = String(body.deviceRingStrategy).trim().toUpperCase();
    if (!VALID_DEVICE_RING_STRATEGIES.has(strategy)) {
      throw Object.assign(
        new Error('deviceRingStrategy must be SIMULTANEOUS, DESK_FIRST, MOBILE_FIRST, DESK_ONLY, or MOBILE_ONLY'),
        { status: 400 },
      );
    }
    data.deviceRingStrategy = strategy;
  }
  ...
  if (body.userId) {
    await prisma.$transaction(async (tx) => {
      await releaseEmployeeFromOtherExtensions(tx, tenantId, body.userId, extensionId);
      await tx.extension.update({ where: { id: extensionId }, data });
    });
  } else {
...
async function deleteExtension(prisma, tenantId, extensionId) {
  ...
  const { collectExtensionDeletionBlockers } = require('./extensionDependencies');
  const dependencies = await collectExtensionDeletionBlockers(prisma, tenantId, existing);
  if (dependencies.length) {
    throw Object.assign(
      new Error('Extension cannot be deleted while it is referenced by other routing configuration'),
      { status: 409, code: 'EXTENSION_HAS_DEPENDENCIES', dependencies },
    );
  }
