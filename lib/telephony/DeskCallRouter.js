...
async function resolveDeskExtensionNumber(prisma, payload, platform, options, deps) {
  ...
  if (extensionNumber) {
    return { extensionNumber, caller: options.caller ?? null };
  }

  if (!isTelnyxCredentialSipDestination(payload?.to)) {
    return { extensionNumber: null, caller: options.caller ?? null };
  }

  let caller = options.caller ?? null;
  if (!caller?.tenantId && !options.callerProvided) {
    caller = await deps.resolveCaller(prisma, payload, platform, { logCallerResolved: true });
  }
  if (!caller?.tenantId) {
    return { extensionNumber: null, caller };
  }

  extensionNumber = await resolveExtensionNumberFromTo(prisma, payload?.to, caller.tenantId);
