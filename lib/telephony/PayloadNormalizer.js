...
function isV3CallControlApplicationOutbound(payload) {
  ...
  const payloadConnectionId = payload?.connection_id ? String(payload.connection_id) : null;
  if (!v3CallControlApplicationId || !payloadConnectionId) return false;
  if (!isOutboundDirection(payload?.direction)) return false;
  return payloadConnectionId === String(v3CallControlApplicationId);
}

/**
 * Grandstream desk outbound with Telnyx call parking (Pattern 1).
 * Credential Connection + state=parked — distinct from mobile/WebRTC credential outbound.
 */
function isDeskCredentialParkedOutbound(payload, platform) {
  if (!isDeskOriginatedParkedOutbound(payload)) return false;
  return describeCredentialConnectionOutboundGate(payload, platform).acceptedAs === 'Credential Connection';
}

/**
 * Desk Call Router V2 eligibility: legacy CC app, V3 CC app (parked), or credential parked desk leg.
 */
function isDeskOutboundV2Eligible(payload, platform) {
  if (!isOutboundDirection(payload?.direction)) return false;
  if (isCallControlApplicationOutbound(payload, platform)) return true;
  if (isV3CallControlApplicationOutbound(payload) && isDeskOriginatedParkedOutbound(payload)) {
    return true;
  }
  return isDeskCredentialParkedOutbound(payload, platform);
}
...
module.exports = {
  ...
  isDeskCredentialParkedOutbound,
  isDeskOutboundV2Eligible,
