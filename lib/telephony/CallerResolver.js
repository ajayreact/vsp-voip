...
/**
 * Valid E.164 caller ID for ringing another extension (desk/app).
 * Internal sessions use from labels like ext:100 which Telnyx rejects on dial legs.
 */
async function resolveExtensionE164CallerId(prisma, tenantId, extension) {
  const { normalizePhoneNumber } = require('../phone');
  const { loadExtensionPhoneNumber } = require('../extensionOwnership');

  if (!prisma || !tenantId || !extension?.id) return null;

  const row = extension.security
    ? extension
    : await prisma.extension.findFirst({
      where: { id: extension.id, tenantId },
      include: { security: true, primaryPhoneNumber: true },
    });
  if (!row) return null;

  const outboundCallerId = row.security?.outboundCallerId;
  if (isValidE164CallerId(outboundCallerId)) {
    return normalizePhoneNumber(outboundCallerId);
  }

  let assignedDid = row.primaryPhoneNumber?.number || null;
  if (!assignedDid) {
    assignedDid = (await loadExtensionPhoneNumber(prisma, row.id))?.number || null;
  }
  if (isValidE164CallerId(assignedDid)) {
    return normalizePhoneNumber(assignedDid);
  }
