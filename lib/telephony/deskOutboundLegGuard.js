...
async function markDeskOutboundChildLeg(callControlId) {
  ...
  // TEMPORARY DIAGNOSTIC LOGGING — remove once redial-loop investigation is closed.
  console.log(`MARK CHILD LEG ${callControlId}`, { backend: redis ? 'redis' : 'memory', key });
  if (redis) {
...
async function isDeskOutboundChildLeg(callControlId) {
  ...
  if (redis) {
    ...
    // TEMPORARY DIAGNOSTIC LOGGING — remove once redial-loop investigation is closed.
    console.log(`CHECK CHILD LEG ${callControlId} FOUND=${found}`, { backend: 'redis', key });
    return found;
  }

  pruneMemoryMarks();
  const found = memoryMarks.has(key);
  // TEMPORARY DIAGNOSTIC LOGGING — remove once redial-loop investigation is closed.
  console.log(`CHECK CHILD LEG ${callControlId} FOUND=${found}`, { backend: 'memory', key, memorySize: memoryMarks.size });
  return found;
}
