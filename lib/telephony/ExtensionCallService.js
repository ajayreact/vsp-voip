...
async function handleExtensionOutbound(ctx, deps = defaultDeps) {
  ...
  const { traceOutbound, snapshotCaller } = require('./outboundRuntimeTrace');
  ...
  // Block/voicemail/forward policies speak or route immediately and need an
  // answered leg for media, so answer up front for those only. The default
  // ring path (policy.action === 'ring') keeps the caller leg parked —
  // beginInternalExtensionRing defers the answer (ring-first) until the
  // target extension actually answers, same pattern as inbound PSTN ring-first.
  let preAnswered = false;
  if (policy?.action && policy.action !== 'ring') {
