...
async function handleExtensionOutbound(ctx, deps = defaultDeps) {
  ...
  console.log('TRACE handleExtensionOutbound: extension requested', {
    extensionRequested: extensionNumber ?? null,
  ...
  // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
  console.log('TRACE handleExtensionOutbound: destination extension lookup', {
    extensionRequested: extensionNumber ?? null,
    found: Boolean(targetExtension),
    targetExtensionId: targetExtension?.id ?? null,
    targetExtensionUserId: targetExtension?.userId ?? null,
    deskDevice_telnyxSipUsername: targetExtension?.telnyxSipUsername ?? null,
  });
  ...
  if (!ctx.skipMobileGate) {
    ...
    if (deps.hasAppRingTargets(ringPreview?.targets || [])) {
      // TEMP TRACE (desk->desk voicemail investigation) — remove after root cause confirmed.
      console.log('TRACE handleExtensionOutbound: routing decision — falling back to legacy/mobile handler', {
        extensionRequested: extensionNumber ?? null,
        reason: 'mobile_app_targets_present',
        ringPreviewTargets: (ringPreview?.targets || []).map((t) => t.type),
      });
      logDeskOutboundRoute({
        version: 'V2',
        destination: 'EXTENSION',
