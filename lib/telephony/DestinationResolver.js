...
/**
 * Resolve target extension by credential username when caller tenant is unknown.
 * Telnyx gencred usernames are unique per account; used for internal URI destinations.
 */
async function loadTargetExtensionBySipUsernameGlobal(prisma, sipUsername) {
  if (!prisma || !sipUsername) return null;
  ...
  if (user?.extensions?.[0]) {
    return { extension: user.extensions[0], tenantId: user.tenantId };
  }

  const legacyExtension = await prisma.extension.findFirst({
    where: {
      telnyxSipUsername: { equals: sipUsername, mode: 'insensitive' },
      status: 'ACTIVE',
    },
  ...
  if (!legacyExtension) return null;
  return { extension: legacyExtension, tenantId: legacyExtension.tenantId };
}
...
async function resolveExtensionNumberFromTo(prisma, to, tenantId) {
  ...
  const sipUsername = parseCredentialSipUsername(to);
  if (!sipUsername || !prisma) return null;

  if (tenantId) {
    const extension = await loadTargetExtensionBySipUsername(prisma, tenantId, sipUsername);
    if (extension?.extensionNumber) return extension.extensionNumber;
  }

  const global = await loadTargetExtensionBySipUsernameGlobal(prisma, sipUsername);
  return global?.extension?.extensionNumber ?? null;
}
...
async function resolveOutboundDestination(prisma, payload, caller) {
  ...
  try {
    const { traceDeskDesk } = require('./deskDeskTrace');
    traceDeskDesk('resolveOutboundDestination()', {
      tenantId,
      to,
      from: payload?.from ?? null,
      callControlId: payload?.call_control_id ?? null,
      callSessionId: payload?.call_session_id ?? null,
      destinationKind: classified.kind,
      targetExtension: classified.extensionNumber ?? null,
      sipUsername: classified.sipUsername ?? null,
      pstnNumber: classified.pstnNumber ?? null,
    });
  } catch {
    // trace optional — never affects resolution
  }
  ...
  if (classified.kind === 'CREDENTIAL_SIP' && prisma) {
    let extension = null;
    let resolvedTenantId = tenantId;

    if (tenantId) {
      extension = await loadTargetExtensionBySipUsername(
        prisma,
        tenantId,
    ...
    if (!extension?.extensionNumber) {
      const global = await loadTargetExtensionBySipUsernameGlobal(prisma, classified.sipUsername);
      if (global?.extension?.extensionNumber) {
        extension = global.extension;
        resolvedTenantId = global.tenantId ?? tenantId;
      }
    }
    if (extension?.extensionNumber) {
      return {
        ...
        resolvedVia: tenantId ? 'credential_sip' : 'credential_sip_global',
...
async function loadTargetExtension(prisma, tenantId, extensionNumber) {
  ...
  try {
    const { traceDeskDesk } = require('./deskDeskTrace');
    traceDeskDesk('loadTargetExtension()', {
      tenantId,
      extension: extensionNumber,
      targetExtension: extensionNumber,
      found: Boolean(row),
      extensionUserId: row?.userId ?? null,
    });
  } catch {
    // trace optional
  }
...
module.exports = {
  ...
  loadTargetExtensionBySipUsernameGlobal,
