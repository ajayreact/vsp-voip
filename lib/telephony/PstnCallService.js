...
const { markDeskOutboundChildLeg } = require('./deskOutboundLegGuard');
...
async function handlePstnOutbound(ctx) {
  ...
  try {
    ...
    // Record the new leg's call_control_id as "created by us" before its own
    // call.initiated webhook can arrive, so it isn't mistaken for a fresh
    // desk-originated call (see deskOutboundLegGuard.js for the full story).
