...
async function syncTenantPhoneExtensionLinks(prisma, tenantId) {
  ...
  for (const extension of extensions) {
    ...
    if (extRow) {
      await ensureEmployeeTelephonyForExtension(prisma, extRow);
    }
  }
...
async function resolveOwnershipChain(prisma, tenantId, phoneNumberOrId) {
  ...
  const hasEmployeeCredentials = Boolean(employee?.telnyxCredentialId && employee?.telnyxSipUsername);
  const hasLegacyDeskCredentials = Boolean(
    extension?.telnyxCredentialId && extension?.telnyxSipUsername,
  );
  const hasAppCredentials = hasEmployeeCredentials;
  const hasSipCredentials = hasEmployeeCredentials || hasLegacyDeskCredentials;
  ...
  return {
    ...
    extension: extension
      ? {
        ...
        sipUsername: employee?.telnyxSipUsername || extension.telnyxSipUsername || extension.sipUsername || extension.extensionNumber,
        hasSipCredentials: hasEmployeeCredentials || hasLegacyDeskCredentials,
        deskRegistered: Boolean(employee?.sipRegistered || extension?.sipRegistered),
      }
      : null,
    employee: employee
      ? {
        ...
        sipUsername: employee?.telnyxSipUsername || extension?.telnyxSipUsername || extension?.sipUsername || null,
        hasCredential: hasEmployeeCredentials || Boolean(employee.telnyxCredentialId),
        deskRegistered: Boolean(employee?.sipRegistered || extension?.sipRegistered),
