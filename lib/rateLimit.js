...
/** Production keeps strict limits; dev/QA allows parallel test workers without 429 noise. */
function effectiveMax(max) {
  if (process.env.NODE_ENV === 'production') return max;
...
/** V3 repair / inspect endpoints — moderate throttle per user. */
const v3RepairLimiter = rateLimitMiddleware({
  name: 'v3-repair',
  windowMs: 60 * 1000,
  max: 20,
  keyFn: (req) => req.user?.sub || req.ip || 'unknown',
...
/** V3 heavy mutations — runtime resync, migration run, test lab. */
const v3HeavyMutationLimiter = rateLimitMiddleware({
  name: 'v3-heavy',
  windowMs: 60 * 1000,
  max: 8,
  keyFn: (req) => req.user?.sub || req.ip || 'unknown',
