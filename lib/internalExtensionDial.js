...
async function beginInternalExtensionRing(
  ...
  const { resolveExtensionE164CallerId } = require('./telephony/CallerResolver');
  const agentRingFrom = await resolveExtensionE164CallerId(prisma, tenant.id, callerExtension);
  ...
  const session = {
    ...
    from: callerFrom,
    agentRingFrom: agentRingFrom || null,
    callerDisplayName: callerExtension?.displayName || callerFrom,
    to: vmTo,
    credentialConnectionId,
    callControlApplicationId,
...
async function handleInternalRingGroupCallInitiated(
  ...
  const { resolveExtensionE164CallerId } = require('./telephony/CallerResolver');
  const agentRingFrom = await resolveExtensionE164CallerId(prisma, tenant.id, caller.callerExtension);
  ...
  const session = {
    ...
    from: callerFrom,
    agentRingFrom: agentRingFrom || null,
    callerDisplayName: caller.callerExtension?.displayName || callerFrom,
...
async function handleParkedWebRtcOutboundInitiated(prisma, payload, platform) {
  ...
  // Desk -> PSTN re-dial loop fix: skip legs we created ourselves via
  // PstnCallService.handlePstnOutbound (dialDestination()'s own call.initiated
  // for the new leg looks identical to a fresh desk call — same connection_id).
  // See deskOutboundLegGuard.js for the full root-cause explanation.
  const { isDeskOutboundChildLeg } = require('./telephony/deskOutboundLegGuard');
  if (await isDeskOutboundChildLeg(callControlId)) {
    console.log('   ↳ Parked WebRTC outbound skipped: leg was created by our own dialDestination() call — not a new desk call', {
      callControlId,
    });
    logDeskOutboundRoute({
      event: 'desk.outbound.route',
      result: 'skipped',
      reason: 'self_dialed_child_leg',
      callControlId,
    });
    return false;
  }
