...
} = require('./telephony/deskOutboundTrace');
const {
  isPstnDestination,
...
async function beginInternalExtensionRing(
  ...
  } = require('./telephony/outboundRuntimeTrace');
  const preResolvedTargets = resolution?.targets || [];
  traceOutbound('beginInternalExtensionRing', {
    flow: 'desk_to_desk',
    callControlId,
    targetExtensionNumber: targetExtension.extensionNumber,
    targetExtensionId: targetExtension.id,
    targetUserId: targetExtension.userId ?? null,
    targetSipUsername: targetExtension.user?.telnyxSipUsername
      ?? targetExtension.telnyxSipUsername
      ?? null,
    preResolvedTargetsCount: preResolvedTargets.length,
    preResolvedTargetTypes: preResolvedTargets.map((t) => t.type),
    preResolvedDialTos: preResolvedTargets.map((t) => {
      const { formatTargetDialTo } = require('./inboundRouting');
      return formatTargetDialTo(t);
    }),
    blocking: preResolvedTargets.length === 0,
    blockingReason: preResolvedTargets.length === 0
      ? `extension ${targetExtension.extensionNumber} has no ring targets (no user.telnyxSipUsername and no legacy extension.telnyxSipUsername)`
      : null,
  });

  traceDeskDesk('beginInternalExtensionRing()', {
    tenantId: tenant?.id ?? null,
    targetExtension: targetExtension.extensionNumber,
    callControlId,
    callSessionId: callSessionId || callControlId,
    targetCount: resolution?.targets?.length ?? 0,
    targets: (resolution?.targets || []).map((t) => t.type),
  });

  const vmTo = buildExtensionVoicemailToLabel(targetExtension.extensionNumber)
    || targetExtension.extensionNumber;

  // Ring-first: when there's an actual target to ring and the caller leg
  // isn't already answered, keep it parked until the target answers —
  // reuses the same deferPstnAnswerUntilAgent + bridgeParkedInboundToAgentLeg
  // mechanism as inbound PSTN ring-first (docs/vsp/pbx/21-event-sequence.md).
  // This stops the caller's phone timer from starting on 180 Ringing.
  ...
  if (!session.preResolvedTargets.length) {
    traceOutboundIfInternalSession(session, 'beginInternalExtensionRing_blocked', {
      blocking: true,
      blockingReason: 'preResolvedTargets_empty',
      preResolvedTargetsCount: 0,
    });
    if (!alreadyAnswered) {
  ...
  traceOutboundIfInternalSession(session, 'startConnectFlow_enter', {
    ringTargetCount: session.preResolvedTargets.length,
  });
  await getStartConnectFlow()(session, prisma, { skipAnnouncements: true });
  return session;
}
...
async function handleInternalExtensionCallInitiated(prisma, payload, platform, targetExtensionNumber) {
  ...
  const caller = await resolveCallerFromPayload(prisma, payload, platform, {
    traceOutbound: true,
    traceFlow: 'desk_to_desk',
  });

  traceOutbound('caller_resolution', {
    flow: 'desk_to_desk',
    callControlId,
    targetExtensionNumber,
    ...snapshotCaller(caller),
    blocking: !caller?.tenantId,
    blockingReason: !caller?.tenantId ? 'caller_not_resolved' : null,
  });
  ...
  if (!caller?.tenantId) {
    traceDeskDesk('handleInternalExtensionCallInitiated.caller_not_resolved', {
      callControlId,
      targetExtension: targetExtensionNumber,
      from: payload?.from ?? null,
      to: payload?.to ?? null,
    });
  ...
  traceOutbound('target_extension_lookup', {
    flow: 'desk_to_desk',
    callControlId,
    targetExtensionNumber,
    tenantId: tenant.id,
    found: Boolean(targetExtension),
    targetExtensionId: targetExtension?.id ?? null,
    targetUserId: targetExtension?.userId ?? null,
    targetSipUsername: targetExtension?.user?.telnyxSipUsername
      ?? targetExtension?.telnyxSipUsername
      ?? null,
    blocking: !targetExtension,
    blockingReason: !targetExtension ? `extension ${targetExtensionNumber} not found for tenant ${tenant.id}` : null,
  });
...
async function handleParkedWebRtcOutboundInitiated(prisma, payload, platform) {
  ...
  const flowKind = isPstnDestination(payload?.to) ? 'desk_to_pstn' : 'desk_to_desk';

  traceOutbound('sip_invite_received', {
    flow: flowKind,
    callControlId,
    note: 'Telnyx parked call.initiated implies desk SIP INVITE was accepted',
    from: payload?.from ?? null,
    to: payload?.to ?? null,
    connection_id: payload?.connection_id ?? null,
    direction: payload?.direction ?? null,
  ...
  traceOutbound('handleParkedWebRtcOutboundInitiated_entered', {
    flow: flowKind,
    callControlId,
    from: payload?.from ?? null,
    to: payload?.to ?? null,
  ...
  traceDeskDesk('handleParkedWebRtcOutboundInitiated.enter', {
    callControlId,
  ...
  if (isV3CallControlApplicationOutbound(payload)) {
    ...
    traceOutbound('v3_bridge_decision', {
      flow: flowKind,
      callControlId,
      connection_id: payload?.connection_id ?? null,
      shouldBridge: bridgeDecision.shouldBridge,
      bridgeReason: bridgeDecision.reason,
      bridgeTenantId: bridgeDecision.tenantId ?? null,
    });
    if (bridgeDecision.shouldBridge) {
      console.log('[CALL CONTROL] parked outbound skipped — V3 Call Control app handles this call', {
        callControlId,
        connection_id: payload?.connection_id || null,
    ...
    console.log('[CALL CONTROL] V3 app parked outbound — legacy routing (tenant V3 desk disabled)', {
      callControlId,
      connection_id: payload?.connection_id || null,
      tenantId: bridgeDecision.tenantId,
  ...
  logDeskOriginatedTrace('handler_enter', payload, {
  ...
  if (await handleInternalApiCallerInitiated(prisma, payload, platform)) {
    logDeskOriginatedTrace('exit_internal_api', payload, { result: 'internal_api' });
    logDeskOutboundRoute({ event: 'desk.outbound.route', result: 'internal_api', callControlId });
    return true;
  }

  const outboundGate = logDeskOriginatedOutboundGate(payload, platform)
    ?? describeCredentialConnectionOutboundGate(payload, platform);
  traceOutbound('outbound_gate', {
    flow: flowKind,
    callControlId,
    ...snapshotOutboundGate(outboundGate, payload, platform),
    blocking: !outboundGate.ok,
    blockingReason: !outboundGate.ok ? (
      outboundGate.reason === 'connection_id_mismatch'
        ? `payload connection_id ${payload?.connection_id ?? 'null'} matched neither credential connection ${outboundGate.expectedCredentialConnectionId ?? 'null'} nor legacy call control app ${outboundGate.expectedCallControlApplicationId ?? 'null'}`
        : outboundGate.reason
    ) : null,
  });
  if (!outboundGate.ok) {
    console.warn('   ↳ Parked WebRTC outbound skipped:', outboundGate);
    logDeskOriginatedTrace('exit_outbound_gate_failed', payload, {
      outboundGateOk: false,
      outboundGateReason: outboundGate.reason,
      routeDeskOutboundCalled: false,
      routeDeskOutboundSkipReason: `outbound_gate_${outboundGate.reason}`,
    });
    logDeskOutboundRoute({
      event: 'desk.outbound.route',
      result: 'skipped',
  ...
  const { isCallControlApplicationOutbound, isDeskOutboundV2Eligible } = require('./telephony/PayloadNormalizer');
  ...
  if (isPstnDestination(payload.to)) {
    ...
    traceOutbound('caller_resolution', {
      ...
      ...snapshotCaller(resolvedCaller),
      blocking: !resolvedCaller?.tenantId,
      blockingReason: !resolvedCaller?.tenantId ? 'caller_not_resolved' : null,
    });
  ...
  if (!extensionNumber && isTelnyxCredentialSipDestination(payload.to)) {
    ...
    traceOutbound('resolveExtensionNumberFromTo', {
      flow: flowKind,
      callControlId,
      to: payload?.to ?? null,
      callerTenantId: resolvedCaller?.tenantId ?? null,
      extensionNumber: extensionNumber ?? null,
      resolved: Boolean(extensionNumber),
      blocking: !extensionNumber,
      blockingReason: !extensionNumber
        ? `could not map credential SIP destination ${payload?.to ?? 'null'} to extension digits`
        : null,
    });
    if (extensionNumber) {
      logDeskOriginatedTrace('destination_resolved', payload, {
        extensionNumber,
  ...
  const deskV2Eligible = isDeskOutboundV2Eligible(payload, platform);
  const ccAppOutbound = isCallControlApplicationOutbound(payload, platform);

  if (v2Enabled && deskV2Eligible) {
    ...
    logDeskOriginatedRouteDeskOutboundDecision(payload, platform, {
      outboundGateOk: true,
      outboundGateReason: outboundGate.reason,
      extensionNumber,
    ...
    traceOutbound('selected_outbound_route', {
      flow: extensionNumber ? 'desk_to_desk' : 'desk_to_pstn',
      callControlId,
      route: 'v2_routeDeskOutbound',
      v2Enabled,
      ccAppOutbound,
      deskCredentialParked: deskV2Eligible && !ccAppOutbound,
      extensionNumber: extensionNumber ?? null,
      destinationNumber: payload?.to ?? null,
      ...snapshotCaller(resolvedCaller),
    });
    ...
    traceOutbound('routeDeskOutbound_result', {
      flow: extensionNumber ? 'desk_to_desk' : 'desk_to_pstn',
      callControlId,
      v2Result,
      extensionNumber: extensionNumber ?? null,
      handled: v2Result !== null,
    });

    logDeskOriginatedTrace('routeDeskOutbound_result', payload, {
      routeDeskOutboundCalled: true,
      v2Result,
      extensionNumber,
      callerResolved: Boolean(resolvedCaller?.tenantId),
    });
    ...
    if (v2Result !== null) {
      ...
      logDeskOriginatedTrace(v2Result ? 'exit_v2_handled' : 'exit_v2_failed', payload, {
        routing: 'v2',
        v2Result,
      });
      logDeskOutboundRoute({
  ...
  if (extensionNumber) {
    ...
    logDeskOriginatedTrace('exit_legacy_extension', payload, { handled, extensionNumber });
    return true;
  }

  if (isPstnDestination(payload.to)) {
    logDeskOriginatedTrace('exit_legacy_pstn_passthrough', payload, {});
    return handleParkedPstnOutboundPassthrough(prisma, payload, platform);
  }

  traceOutbound('unsupported_destination', {
    flow: flowKind,
    callControlId,
    to: payload?.to ?? null,
    credentialSip: isTelnyxCredentialSipDestination(payload?.to),
    pstn: isPstnDestination(payload?.to),
    extensionDigits: parseInternalExtensionDestination(payload?.to),
    ...snapshotCaller(resolvedCaller),
    blocking: true,
    blockingReason: 'unsupported_destination',
    why: `to=${payload?.to ?? 'null'} is not PSTN, not extension digits, and credential SIP did not resolve to an extension`,
  });

  logDeskOriginatedTrace('exit_unsupported_destination', payload, {
    routeDeskOutboundCalled: false,
