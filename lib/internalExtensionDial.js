 ...
 * @param {string} [handlerOptions.webhookSource]
 */
async function handleParkedWebRtcOutboundInitiated(prisma, payload, platform, handlerOptions = {}) {
  const callControlId = payload?.call_control_id || null;
  const webhookSource = handlerOptions.webhookSource || 'unknown';

  const {
    describeV3CallControlApplicationOutbound,
  } = require('./telephony/PayloadNormalizer');
  const v3Gate = describeV3CallControlApplicationOutbound(payload);
  const v3Skip = v3Gate.ok;

  console.log('[V3] isV3CallControlApplicationOutbound', {
    connection_id: v3Gate.payloadConnectionId,
    call_control_id: v3Gate.callControlId,
    TELNYX_CALL_CONTROL_APP_ID: v3Gate.TELNYX_CALL_CONTROL_APP_ID,
    TELNYX_V3_CALL_CONTROL_APP_ID: v3Gate.TELNYX_V3_CALL_CONTROL_APP_ID,
    matchV3AppConnectionId: v3Gate.matchV3AppConnectionId,
    matchTelnyxV3CallControlIdPrefix: v3Gate.matchTelnyxV3CallControlIdPrefix,
    returned: v3Skip,
  });

  console.log('[CALL CONTROL] handleParkedWebRtcOutboundInitiated entry', {
    callControlId,
    connection_id: payload?.connection_id || null,
    webhookSource,
    direction: payload?.direction || null,
    state: payload?.state || null,
    v3SkipEvaluated: v3Skip,
    v3SkipAction: v3Skip ? 'skip_legacy_v2' : 'continue_v2',
    v3Gate,
  });
