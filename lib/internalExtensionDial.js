...
async function resolveCallerFromUniqueRegisteredExtension(prisma, extensionNumber) {
  ...
  if (!EXTENSION_DIAL_PATTERN.test(digits)) return null;
...
function describeCredentialConnectionOutboundGate(payload, platform) {
  ...
  if (!credentialConnectionId) {
    return { ok: false, reason: 'missing_platform_credential_connection_id' };
  }
  if (!payloadConnectionId) {
    return { ok: false, reason: 'missing_payload_connection_id' };
  }
  if (payloadConnectionId !== String(credentialConnectionId)) {
    return {
      ok: false,
      reason: 'connection_id_mismatch',
      expectedConnectionId: credentialConnectionId,
      payloadConnectionId,
    };
  }
  if (!isOutboundDirection(direction)) {
    return { ok: false, reason: 'direction_not_outbound', direction };
  }
