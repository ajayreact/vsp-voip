...
const memoryActiveAgentCalls = new Map();
...
function activeAgentCallRedisKey(sipUsername) {
  return `ccs:active:${String(sipUsername || '').trim().toLowerCase()}`;
}
...
async function deleteSession(callControlId) {
  ...
  try {
    const { logDeskTelephonyEvent } = require('./telephony/deskOutboundLogger');
    logDeskTelephonyEvent('session.closed', {
      callControlId: id,
      tenantId: session?.tenantId ?? null,
      callKind: session?.callKind ?? null,
    });
  } catch {
    // logging must not block session cleanup
  }
...
/** Maps agent SIP username → active bridged inbound call (for blind transfer lookup). */
async function indexActiveAgentCall(sipUsername, {
  ...
  const user = String(sipUsername || '').trim().toLowerCase();
  const inboundId = buildSessionKey(inboundCallControlId);
  const legId = buildSessionKey(agentLegId);
  if (!user || !inboundId || !legId) return;

  const payload = JSON.stringify({
    inboundCallControlId: inboundId,
    agentLegId: legId,
    callerLegId: buildSessionKey(callerLegId) || inboundId,
  ...
  const redis = await getRedisClient().catch(() => null);
  if (redis) {
    await redis.set(activeAgentCallRedisKey(user), payload, 'EX', SESSION_TTL_SEC);
    return;
  }
  memoryActiveAgentCalls.set(user, payload);
}

async function resolveActiveAgentCall(sipUsername) {
  const user = String(sipUsername || '').trim().toLowerCase();
  if (!user) return null;

  const redis = await getRedisClient().catch(() => null);
  if (redis) {
    const raw = await redis.get(activeAgentCallRedisKey(user));
    return raw ? JSON.parse(raw) : null;
  }
  const raw = memoryActiveAgentCalls.get(user);
  return raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : null;
}

async function clearActiveAgentCall(sipUsername) {
  const user = String(sipUsername || '').trim().toLowerCase();
  if (!user) return;

  const redis = await getRedisClient().catch(() => null);
  if (redis) {
    await redis.del(activeAgentCallRedisKey(user));
  }
  memoryActiveAgentCalls.delete(user);
}
