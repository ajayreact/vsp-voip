...
async function getExtensionSipCredentials(prisma, tenantId, extensionId) {
  let extension = await loadExtensionWithTenant(prisma, tenantId, extensionId);
  const ensured = await ensureExtensionSipCredentials(prisma, extension);
  if (ensured?.telnyxSipUsername) {
    // Symplore pilot only: ensureExtensionSipCredentials() may have just created a
    // dedicated desk credential on the extension row — merge it in so the rest of
    // this function (and the provisioning XML it produces) sees it immediately,
    // without waiting for the next request to re-load the extension from the DB.
    extension = { ...extension, ...ensured };
  }
  ...
  await ensureExtensionTelephonyProvisioned(prisma, extension, { stage: 'provision-desk-phone' });
  ...
  // Symplore pilot only: when the extension has its own dedicated Telnyx desk
  // credential (see ensureDedicatedDeskTelnyxCredential in lib/extensionSip.js), the
  // physical desk phone must register with THAT credential, not the employee's
  // shared app credential — otherwise the desk phone and the mobile/softphone app
  // keep evicting each other's Telnyx registration (only one device can be
  // registered per credential at a time). Every other tenant is unaffected because
  // extension.telnyxSipUsername stays null under the existing Phase 2.4a behavior.
  const deskCredential = extension.telnyxSipUsername
    ? { telnyxSipUsername: extension.telnyxSipUsername, telnyxSipPassword: extension.telnyxSipPassword }
    : null;
  if (deskCredential) {
    provisioningProfile.sip = buildTelnyxSipBlock(deskCredential, { includeSecrets: true });
  }
  ...
  return {
    ...
    sipUsername: deskCredential?.telnyxSipUsername || telnyxUsername || sipProfile.sipUsername,
    sipPassword: deskCredential?.telnyxSipPassword || refreshedUser?.telnyxSipPassword || sipProfile.sipPassword,
    telnyxSipUsername: deskCredential?.telnyxSipUsername || telnyxUsername,
    loginToken,
