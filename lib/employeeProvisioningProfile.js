...
/**
 * Desk/Grandstream phones register over UDP on the platform's Credential Connection
 * (see buildSipEndpointProfile() in telnyxSipProfile.js, DEFAULT_SIP_PORT-first
 * convention). Defaulting to UDP here keeps outboundProxy/port/transport consistent
 * with that working desk-registration path; passing transport: 'TLS' explicitly
 * remains supported for callers that actually need it.
 */
function buildTelnyxSipBlock(user, { includeSecrets = false, transport = 'UDP' } = {}) {
