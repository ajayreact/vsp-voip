...
async function ensureCredentialConnectionWebhook(connectionId, voiceWebhookUrl) {
  ...
  const existingOutbound = connection?.outbound || {};
  const parkingEnabled = existingOutbound.call_parking_enabled === true;
  const profileId = existingOutbound.outbound_voice_profile_id || null;
  ...
  await telnyxRequest('patch', `/credential_connections/${encodeURIComponent(connectionId)}`, {
    ...
    outbound: {
      ...
      ...(profileId ? { outbound_voice_profile_id: profileId } : {}),
    },
  });

  return { updated: true, webhookUrl: voiceWebhookUrl, callParkingEnabled: true };
}
